/* ==========================================================================
   ...
   Round 4 (owner: "micro-foley IDM" — Paradigm's Fourth Fund film, OpenAI's Super Bowl spot; "it doesn't
   have the kind of mechanical keyboard strokes"): a "Bits & Atoms" family of tactile software and hardware
   sounds, all additions (every earlier name is unchanged):
     bits   play.key({ kind: 'press'|'space'|'backspace'|'enter', vel, tone, up, hold })   mechanical switch
            play.chime(midi, { dur ≤ 1.2, vel, bright })   terminal / "compile success" chime (folds above A5)
            play.compile({ progress, count, gap, midi })  compile / progress tick(s)
            play.ping(midi, { two, dur })                 UI feedback ping
     atoms  play.servo({ dur, from, to, load, settle })   micro-servo whir with gear chatter
            play.pneumatic({ kind: 'release'|'puff'|'hiss', dur, pressure })
            play.arm({ dur, from, to })                   precision robotic-arm glide (S-curve, stepper texture)
            play.snap({ size, metal, double })            metallic snap-fit / latch
            play.relay({ kind: 'on'|'off', size })        relay snap
            cnc({ rate, load, midi }) · fan({ speed })     continuous machining hum / server-fan whir → { set, stop }
            assemblyPhrase()                               the audition phrase (sparse → assembly engine → chime)
   They are modal (an excitation into parallel band-passes) and re-roll every partial per hit. Their excitation
   is a 5-sample pulse, not a one-sample impulse: Chrome starts a buffer source between samples by interpolating
   into it, so a one-sample impulse lands at a random 0–100 % of its height (play.click / tick / ui.* still use
   it; 20 clicks at random sub-sample starts spread ~37 dB in audio-check.html, the new voices ≤ 4 dB).
...
(function () {
  ...
  function createEngine(ctx, opts) {
    ...
    /* ================================================================ BITS & ATOMS (round 4 micro-foley) */
    // The owner's references (Paradigm's Fourth Fund film, OpenAI's Super Bowl spot) replace drum kits with
    // tactile hardware and software sounds. Bits: key (mechanical keyboard switch), chime (terminal /
    // "compile success"), compile (progress tick), ping (UI feedback). Atoms: servo, pneumatic, arm, snap,
    // relay, and the continuous cnc() and fan() beds further down.
    // Mostly modal synthesis: one impulse into parallel band-passes, each a decaying partial (Q = π·f·τ),
    // plus sub-millisecond noise contacts, glides and chopped noise. Every hit re-rolls its partials,
    // delays, levels and pan, so no two are identical. A per-voice "small room" (three darker early
    // reflections, 7–27 ms) keeps them dry but placed. Registers as before: tonal one-shots fold above A5,
    // sustained tones (arm glide, cnc spindle, fan blade-pass) stay at or below A4, modal ring
    // fundamentals stay at or below A6, and partials above 7.5 kHz are dropped.

    // Excitation: a 5-sample raised-cosine pulse (sum 1) instead of a one-sample impulse, followed by 0.6 s
    // of silence (resonators ring out before the source ends; a voice is cleaned up — its nodes
    // disconnected — when its last source ends). Chrome starts a buffer source between samples by
    // interpolating into it, so a one-sample impulse started at an arbitrary time comes out at 0–100 % of
    // its height (measured: a start 0.1 sample past a frame → −20 dB, so per-hit levels spread ~18 dB); a
    // smooth pulse keeps its level (±0.3 dB) wherever it starts, in any browser. Its gentle top (−0.8 dB
    // at 3 kHz, −3.3 dB at 6 kHz) suits these voices. (clickInto / tickInto keep the one-sample impulse:
    // the site's existing clicks were voiced by ear with it.)
    var PULSE = [1 / 12, 3 / 12, 4 / 12, 3 / 12, 1 / 12];
    var IMPL = ctx.createBuffer(1, Math.floor(sr * 0.6), sr);
    IMPL.getChannelData(0).set(PULSE);
    var bpPCache = {};
    // exact output peak of the RBJ band-pass for PULSE (cf. bpPeak)
    function bpPeakP(freq, q) {
      var key = Math.round(freq) + ':' + Math.round(q * 20);
      var c = bpPCache[key];
      if (c) return c;
      var w = (2 * Math.PI * freq) / sr, al = Math.sin(w) / (2 * q), a0 = 1 + al;
      ...
      var x1 = 0, x2 = 0, y1 = 0, y2 = 0, pk = 0;
      for (var n = 0; n < 768; n++) {
        var x = n < PULSE.length ? PULSE[n] : 0, y = b0 * x + b2 * x2 - a1 * y1 - a2 * y2;
        x2 = x1; x1 = x; y2 = y1; y1 = y;
        if (Math.abs(y) > pk) pk = Math.abs(y);
      }
      return (bpPCache[key] = Math.max(pk, 1e-5));
    }
    var MODE_MAX_HZ = 7500;
    // modes: [[freq Hz, tau s (amplitude time-constant), amp], …]; amp = that partial's output peak
    // (× `amp`). The source is owned by the voice.
    function modesInto(v, to, t, modes, amp, pan) {
      var s = ctx.createBufferSource(), sum = null, longest = 0;
      s.buffer = IMPL;
      for (var i = 0; i < modes.length; i++) {
        var md = modes[i], f = md[0], tau = Math.min(md[1], 0.07), a = md[2] * amp;
        if (!(a > 0) || !(f > 30) || f > Math.min(MODE_MAX_HZ, NYQ)) continue;
        var q = clamp(Math.PI * f * tau, 0.5, 300);
        var bp = v.n(F('bandpass', f, q)), g = v.n(G(a / bpPeakP(f, q)));
        if (!sum) sum = v.n(G(1));
        s.connect(bp); bp.connect(g); g.connect(sum);
        if (tau > longest) longest = tau;
      }
      if (!sum) return;
      panTo(v, sum, pan, to);
      v.src(s, t, t + Math.min(IMPL.duration, 0.02 + longest * 8));
    }
    // one dry click (a single resonance), level-stable at any start time (cf. clickInto)
    function clickP(v, to, t, freq, q, amp, pan) {
      freq = clamp(freq, 60, NYQ);
      modesInto(v, to, t, [[freq, clamp(q, 0.5, 90) / (Math.PI * freq), 1]], amp, pan);
    }
    // a contact: a sub-millisecond white-noise burst, band-limited (harder = brighter lpHz)
    function noiseTick(v, to, t, len, hpHz, lpHz, amp, pan) {
      var s = noiseSrc(WHITE);
      var hp = v.n(F('highpass', hpHz, 0.7)), lp = v.n(F('lowpass', Math.min(lpHz, NYQ), 0.7)), g = v.n(G(0));
      g.gain.setValueAtTime(0, t);
      g.gain.linearRampToValueAtTime(amp, t + len * 0.3);
      g.gain.linearRampToValueAtTime(0, t + len);
      s.connect(hp); hp.connect(lp); lp.connect(g);
      panTo(v, g, pan, to);
      v.src(s, t, t + len + 0.02);
    }
    // "Small room": three early reflections (6.5–27 ms, low-passed, spread L / R) from the voice's output
    // to its destination. Per voice, so a bus fade, a voice kill and the mute reach them too.
    // amt = the first reflection's level re the direct sound.
    function roomInto(v, amt, lpHz) {
      if (!(amt > 0)) return;
      var lp = v.n(F('lowpass', num(lpHz, 3600), 0.5));
      v.out.connect(lp);
      var taps = [[rand(0.0065, 0.009), rand(-0.7, -0.4), 1], [rand(0.011, 0.016), rand(0.4, 0.7), 0.72], [rand(0.019, 0.027), rand(-0.3, 0.3), 0.42]];
      for (var i = 0; i < taps.length; i++) {
        var d = v.n(ctx.createDelay(0.05)), g = v.n(G(Math.min(amt, 0.6) * taps[i][2])), p = v.n(PAN(taps[i][1]));
        d.delayTime.value = taps[i][0];
        lp.connect(d); d.connect(g); g.connect(p); p.connect(v.dest);
      }
    }
    function jit(x, p) { return x * rand(1 - p, 1 + p); }
    function sstep(x) { x = clamp(x, 0, 1); return x * x * (3 - 2 * x); }

    /* ---------------- bits (software) */

    // Mechanical keyboard switch. { kind: 'press' (default) | 'space' | 'backspace' | 'enter', vel 0..1 (0.8),
    //   tone 0..1 (0.35; 0 = deep "thock", 1 = bright "clack"), up (upstroke level re the press, 0.3;
    //   0 = none), hold (s from bottom-out to the upstroke; default per kind, 60–190 ms), room (0.16),
    //   gain, pan, when }
    // A press = the stem's click at `when`, the bottom-out 3–8 ms later (keycap / housing / plate modes and
    // a short case resonance, rung by an impulse, plus a sub-millisecond noise contact), and a lighter,
    // higher upstroke when the key is let go. Space / backspace / enter are wider keys on stabilizers:
    // lower modes, a flam (the two ends land 1–5 ms apart) and a wire rattle.
    var KEYK = {
      press:     { body: 430, cse: 205, mid: 980, clk: 2050, tik: 3600, bt: 0.010, ct: 0.018, lvl: 1,    flam: [0, 0],           rat: [0, 0], hold: [0.06, 0.12], pre: 0.3,  pan: 0 },
      space:     { body: 255, cse: 150, mid: 540, clk: 1350, tik: 2700, bt: 0.018, ct: 0.026, lvl: 1.3,  flam: [0.0018, 0.0045], rat: [2, 4], hold: [0.08, 0.15], pre: 0.16, pan: 0 },
      backspace: { body: 340, cse: 180, mid: 760, clk: 1750, tik: 3200, bt: 0.013, ct: 0.021, lvl: 1.12, flam: [0.0008, 0.002],  rat: [1, 2], hold: [0.06, 0.11], pre: 0.24, pan: 0.32 },
      enter:     { body: 305, cse: 170, mid: 660, clk: 1600, tik: 3000, bt: 0.014, ct: 0.022, lvl: 1.18, flam: [0.001, 0.0025],  rat: [1, 2], hold: [0.11, 0.19], pre: 0.22, pan: 0.36 }
    };
    P.key = function (o) {
      o = o || {};
      var K = KEYK[o.kind] || KEYK.press;
      var vel = clamp(num(o.vel, 0.8), 0, 1), tone = clamp(num(o.tone, 0.35), 0, 1);
      var v = Voice(o, { level: 0.115 * K.lvl, pan: K.pan + rand(-0.18, 0.18), reverb: 0.02, delay: 0 });
      var t = v.t;
      var a = (0.3 + 0.7 * Math.pow(vel, 1.2)) * rand(0.88, 1.1);                          // hit strength
      var br = clamp(0.2 + 0.55 * vel + 0.5 * (tone - 0.35) + rand(-0.08, 0.08), 0, 1);     // brightness
      var up = 1 + 0.16 * tone, lowW = 1.15 - 0.6 * tone, hiW = 0.45 + 0.9 * br;
      // 1) the stem / leaf click while the key travels (the "press click")
      var tb = t + rand(0.003, 0.008);
      clickP(v, v.out, t, rand(2300, 3300) * up, rand(2.2, 3.6), K.pre * a * (0.6 + 0.6 * br), rand(-0.1, 0.1));
      // 2) the bottom-out
      function bottom(tt, k, hi) {
        modesInto(v, v.out, tt, [
          [jit(K.body * up, 0.05), jit(K.bt, 0.15), 0.62 * lowW],     // keycap + switch "thock"
          [jit(K.body * up * 1.17, 0.04), jit(K.bt * 0.7, 0.15), 0.3 * lowW], // …its neighbour (a thud, not a pitch)
          [jit(K.cse, 0.06), jit(K.ct, 0.15), 0.3 * lowW],            // the case
          [jit(K.mid * up, 0.05), jit(0.0055, 0.2), 0.36],            // plate
          [jit(K.clk * up, 0.06), jit(0.0032, 0.2), 0.5 * hiW * hi],  // keycap "clack"
          [jit(K.tik * up, 0.07), jit(0.0012, 0.25), 0.26 * hiW * hi] // the housing's edge
        ], a * k);
        noiseTick(v, v.out, tt, rand(0.0005, 0.0011), 700, 2200 + 3800 * br, 0.5 * a * k * (0.5 + br));
      }
      bottom(tb, 1, 1);
      if (K.flam[1] > 0) bottom(tb + rand(K.flam[0], K.flam[1]), rand(0.45, 0.75), 0.7);
      // 3) stabilizer wire rattle (wide keys)
      var nr = K.rat[1] > 0 ? Math.round(rand(K.rat[0], K.rat[1])) : 0, tr = tb + rand(0.005, 0.01);
      for (var i = 0; i < nr; i++) {
        clickP(v, v.out, tr, rand(2200, 3500), rand(5, 9), a * rand(0.05, 0.11) * (1 - i * 0.18), rand(-0.3, 0.3));
        tr += rand(0.004, 0.011) * (1 + i * 0.4);
      }
      // 4) the upstroke (top-out) as the key is released: lighter, a little higher, drier
      var upL = clamp(num(o.up, 0.3), 0, 1.5);
      if (upL > 0) {
        var tu = tb + Math.max(0.02, num(o.hold, rand(K.hold[0], K.hold[1])));
        var au = a * upL * rand(0.8, 1.15) * (nr > 0 ? 1.15 : 1);
        modesInto(v, v.out, tu, [
          [jit(K.body * up * 1.22, 0.05), jit(K.bt * 0.45, 0.2), 0.28 * lowW],
          [jit(K.mid * up * 1.15, 0.05), jit(0.004, 0.2), 0.34],
          [jit(K.clk * up * 1.18, 0.06), jit(0.0026, 0.2), 0.55 * hiW],
          [jit(K.tik * up * 1.08, 0.07), jit(0.001, 0.25), 0.3 * hiW]
        ], au);
        noiseTick(v, v.out, tu, rand(0.0004, 0.0008), 900, 2600 + 3000 * br, 0.35 * au);
        if (nr > 0) clickP(v, v.out, tu + rand(0.006, 0.012), rand(2400, 3400), rand(5, 8), au * rand(0.06, 0.1), rand(-0.3, 0.3));
      }
      roomInto(v, num(o.room, 0.16), 3400);
      v.commit();
    };

    // Terminal / "compile success" chime: clean and short. Two sines (±2 cents, a little width) with a glassy
    // FM onset (ratio 3.5, the index falling to 3 % in ~20 ms), an octave partial and a faint inharmonic
    // glass partial (2.76×) that die much sooner; 2 ms attack and an exponential decay from the start —
    // no sustain, no vibrato (not a flute) — reaching −60 dB at `dur` (≤ 1.2 s). Folds above A5.
    // { dur (0.9), vel 0..1 (0.8), bright 0..1 (0.45: the glass), gain, pan, when }
    P.chime = function (midi, o) {
      if (midi && typeof midi === 'object' && !o) { o = midi; midi = o.midi; } // chime({ midi, … }) works too
      o = o || {}; midi = num(midi, NaN); if (midi !== midi) return;
      var v = Voice(o, { level: 0.2, reverb: 0.1, delay: 0.06, prio: 1 });
      var t = v.t, f = midiToHz(foldIf(midi, o, TONAL_MAX)), dur = clamp(num(o.dur, 0.9), 0.12, 1.2);
      var vel = clamp(num(o.vel, 0.8), 0, 1), br = clamp(num(o.bright, 0.45), 0, 1), tau = dur / 6.9, vl = 0.3 + 0.7 * vel;
      var lp = v.n(F('lowpass', Math.min(5000, f * 6), 0.5));
      lp.connect(v.out);
      var amp = v.n(G(0));
      amp.gain.setValueAtTime(0, t);
      amp.gain.linearRampToValueAtTime(vl, t + 0.002);
      amp.gain.setTargetAtTime(0, t + 0.002, tau);
      amp.connect(lp);
      var stop = t + dur + 0.03;
      var mod = O('sine', f * 3.5), mg = v.n(G(0)), I = f * 3.5 * (0.15 + 0.75 * br) * (0.6 + 0.4 * vel);
      mg.gain.setValueAtTime(I, t);
      mg.gain.setTargetAtTime(I * 0.03, t, 0.012 + 0.012 * br);
      mod.connect(mg);
      for (var s = -1; s <= 1; s += 2) {
        var c = O('sine', f, 2 * s), p = v.n(PAN(0.12 * s)), g = v.n(G(0.5));
        mg.connect(c.frequency);
        c.connect(g); g.connect(p); p.connect(amp);
        v.src(c, t, stop);
      }
      v.src(mod, t, stop);
      tone(v, lp, 'sine', f * 2, t, 0.0015, tau * 0.3, (f * 2 > 1200 ? 0.06 : 0.12) * vl, 0, 0, 3);
      if (f * 2.76 < 5000 && br > 0.02) tone(v, lp, 'sine', f * 2.76, t, 0.001, 0.035, 0.05 * br * vl);
      v.commit();
    };

    // Compile / progress tick: a tiny tuned tick with a dry edge, now and then a 4 ms bit an octave down.
    // { midi (default: from progress), progress 0..1 (random; climbs A5 → A6), count (a quick run, 1–16),
    //   gap (s between run ticks, 0.03), gain, pan, when }. Folds above A6.
    var COMPILE_POOL = [81, 83, 86, 88, 90, 93];
    P.compile = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.074, pan: rand(-0.3, 0.3), reverb: 0.02, delay: 0.04 });
      var n = clamp(Math.round(num(o.count, 1)), 1, 16), gap = clamp(num(o.gap, 0.03), 0.006, 0.5);
      var pr = clamp(num(o.progress, Math.random()), 0, 1), tt = v.t;
      for (var k = 0; k < n; k++) {
        var p = n > 1 ? clamp(pr + (k / (n - 1)) * 0.35, 0, 1) : pr;
        var m = isFinite(o.midi) ? o.midi : COMPILE_POOL[Math.min(COMPILE_POOL.length - 1, Math.floor(p * COMPILE_POOL.length))];
        m = foldIf(m, o, TICK_MAX);
        var a = k === 0 ? 1 : rand(0.6, 0.85);
        var fq = midiToHz(m + rand(-0.04, 0.04));
        clickP(v, v.out, tt, fq, clamp((Math.PI * fq * rand(0.0022, 0.0032)) / 3, 1.5, 90), a);
        clickP(v, v.out, tt, rand(2600, 3600), 1.4, 0.16 * a);
        if (Math.random() < 0.25) bitInto(v, v.out, tt + 0.001, m - 12, 0.004, 0.1 * a, null, 's8', NaN, 3800);
        tt += gap * rand(0.9, 1.1);
      }
      v.commit();
    };

    // UI feedback ping: a round sine (+ a little triangle) that settles UP onto its pitch in 12 ms (blip
    // drops onto its pitch; a ping confirms), 3 ms attack, short decay. { two: semitones } answers with a
    // second ping 55 ms later. Folds above A5. (midi, default D5) { dur (0.16), two, gain, pan, when }
    P.ping = function (midi, o) {
      if (midi && typeof midi === 'object' && !o) { o = midi; midi = o.midi; } // ping({ midi, … }) works too
      o = o || {}; midi = num(midi, 74);
      var v = Voice(o, { level: 0.18, reverb: 0.06, delay: 0.08 });
      var t = v.t, dur = clamp(num(o.dur, 0.16), 0.04, 0.6);
      function one(tt, m, pk) {
        var f = midiToHz(foldIf(m, o, TONAL_MAX));
        var lp = v.n(F('lowpass', Math.min(3400, f * 4), 0.5));
        lp.connect(v.out);
        tone(v, lp, 'sine', f, tt, 0.003, dur / 4, pk, f * 0.94, 0.012);
        tone(v, lp, 'triangle', f, tt, 0.004, dur / 5, pk * 0.16, f * 0.94, 0.012, 4);
        clickP(v, v.out, tt, darkHz(f * 4), 2, 0.03 * pk);
      }
      one(t, midi, 1);
      if (isFinite(o.two) && o.two) one(t + 0.055, midi + o.two, 0.8);
      v.commit();
    };

    /* ---------------- atoms (hardware) */

    // Micro-servo actuation: the motor's buzzy whine (sawtooth + a detuned square) through a nasal gearbox
    // formant, chopped by a ratchety LFO at the gear-mesh rate (gear chatter) with a band-passed rasp. It
    // spins up from below, glides from → to (MIDI), and drops as the controller slows it onto the target;
    // a gear-engage tick, a gear-stop tick, and sometimes a tiny hunting buzz after it lands.
    // { dur (0.28 s), from (57 = A3), to (from + 7), load 0..1 (0.3: lower, rougher, louder), settle (probability
    //   of the hunting buzz, 0.35), room (0.12), gain, pan, when }. Folds above A5 (keeping the glide).
    P.servo = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.14, pan: rand(-0.35, 0.35), reverb: 0.02, delay: 0 });
      var t = v.t, dur = clamp(num(o.dur, 0.28), 0.06, 2.5), load = clamp(num(o.load, 0.3), 0, 1);
      var m0 = num(o.from, 57), m1 = num(o.to, m0 + 7), top = Math.max(m0, m1);
      var sh = (o.allowHigh ? 0 : fold(top, TONAL_MAX) - top) - 1.6 * load + rand(-0.3, 0.3);
      var f0 = midiToHz(m0 + sh), f1 = midiToHz(m1 + sh), fs = f0 * rand(0.66, 0.76);
      var ta = Math.min(0.03, dur * 0.15), td = Math.min(0.05, dur * 0.2);
      function path(p, k) {
        p.setValueAtTime(fs * k, t);
        p.exponentialRampToValueAtTime(f0 * k, t + ta);
        p.exponentialRampToValueAtTime(f1 * k, t + dur - td);
        p.exponentialRampToValueAtTime(f1 * 0.78 * k, t + dur);
      }
      var saw = O('sawtooth', fs), sq = O('square', fs, rand(5, 12)), lfo = O('sawtooth', fs * 0.25);
      path(saw.frequency, 1); path(sq.frequency, 1); path(lfo.frequency, rand(0.22, 0.3) * (1 - 0.15 * load));
      var mix = v.n(G(1)), gs = v.n(G(0.5)), gq = v.n(G(0.2));
      saw.connect(gs); sq.connect(gq); gs.connect(mix); gq.connect(mix);
      var ns = noiseSrc(WHITE), nbp = v.n(F('bandpass', rand(2000, 2600), 1.3)), ng = v.n(G(0.35 + 0.6 * load));
      ns.connect(nbp); nbp.connect(ng); ng.connect(mix);
      var depth = 0.35 + 0.4 * load, am = v.n(G(1 - depth / 2)), amd = v.n(G(depth / 2));
      lfo.connect(amd); amd.connect(am.gain);
      var hp = v.n(F('highpass', 180, 0.7)), fm1 = v.n(F('peaking', rand(1050, 1400), 1.4)), fm2 = v.n(F('peaking', rand(2200, 2800), 2.2));
      var lp = v.n(F('lowpass', 3600, 0.6));
      fm1.gain.value = 7; fm2.gain.value = 3;
      var env = v.n(G(0)), pk = 0.8 + 0.35 * load;
      env.gain.setValueAtTime(0, t);
      env.gain.linearRampToValueAtTime(pk, t + 0.006);
      env.gain.setValueAtTime(pk, t + dur - 0.014);
      env.gain.linearRampToValueAtTime(0, t + dur + 0.002);
      chain(mix, am, hp, fm1, fm2, lp, env, v.out);
      var stop = t + dur + 0.01;
      v.src(saw, t, stop); v.src(sq, t, stop); v.src(lfo, t, stop); v.src(ns, t, stop);
      modesInto(v, v.out, t, [[rand(1500, 2000), 0.0015, 0.4], [rand(420, 520), 0.006, 0.3]], 0.35);          // gears engage
      modesInto(v, v.out, t + dur, [[rand(1400, 1900), 0.002, 0.55], [rand(380, 460), 0.008, 0.4]], 0.45);  // gears stop
      if (Math.random() < clamp(num(o.settle, 0.35), 0, 1)) {
        var hl = v.n(F('lowpass', 3000, 0.7)), hh = v.n(F('highpass', 200, 0.7)), hf = v.n(F('peaking', 1200, 1.4));
        hf.gain.value = 6;
        chain(hl, hh, hf, v.out);
        var nh = Math.random() < 0.5 ? 1 : 2, th = t + dur + rand(0.035, 0.07);
        for (var i = 0; i < nh; i++) {
          var b = O('sawtooth', f1 * rand(0.6, 0.72)), bg = v.n(G(0)), bl = rand(0.012, 0.022), ba = 0.3 * pk * (1 - 0.35 * i);
          bg.gain.setValueAtTime(0, th);
          bg.gain.linearRampToValueAtTime(ba, th + 0.003);
          bg.gain.setValueAtTime(ba, th + bl - 0.004);
          bg.gain.linearRampToValueAtTime(0, th + bl);
          b.connect(bg); bg.connect(hl);
          v.src(b, th, th + bl + 0.005);
          th += bl + rand(0.025, 0.05);
        }
      }
      roomInto(v, num(o.room, 0.12), 3200);
      v.commit();
    };

    // Pneumatic valve + air: a solenoid tick and a low "chk", then the air jet (white noise, high-passed, a
    // band-pass that falls as the pressure drops, low-passed at 5.5 kHz) with a darker "pff" under it.
    // Fast attack, air decay. { kind: 'release' (default) | 'puff' (short) | 'hiss' (soft swell, no valve),
    //   dur (s; 0.38 / 0.09 / 0.8), pressure 0..1 (0.6: brighter, louder), attack (hiss), room (0.1),
    //   gain, pan, when }
    P.pneumatic = function (o) {
      o = o || {};
      var kind = o.kind === 'puff' || o.kind === 'hiss' ? o.kind : 'release';
      var v = Voice(o, { level: 0.2, pan: rand(-0.35, 0.35), reverb: 0.05, delay: 0 });
      var t = v.t, pr = clamp(num(o.pressure, 0.6), 0, 1);
      var dur = clamp(num(o.dur, kind === 'puff' ? 0.09 : kind === 'hiss' ? 0.8 : 0.38), 0.03, 4);
      var atk = kind === 'hiss' ? clamp(num(o.attack, 0.06), 0.005, 2) : rand(0.002, 0.004);
      if (kind !== 'hiss') modesInto(v, v.out, t, [[rand(1700, 2200), 0.0014, 0.45], [rand(700, 900), 0.004, 0.3], [rand(150, 200), 0.012, 0.55]], 0.5);
      var ta = t + (kind === 'hiss' ? 0 : rand(0.004, 0.009)), end = ta + atk + dur;
      var fA = (1900 + 1500 * pr) * rand(0.92, 1.08), fB = fA * rand(0.5, 0.6), pk = 0.55 + 0.45 * pr;
      var s = noiseSrc(WHITE), hp = v.n(F('highpass', 1000, 0.7)), bp = v.n(F('bandpass', fA, 0.85)), lp = v.n(F('lowpass', 5500, 0.5)), g = v.n(G(0));
      bp.frequency.setValueAtTime(fA, ta);
      bp.frequency.exponentialRampToValueAtTime(fB, end);
      g.gain.setValueAtTime(0, ta);
      g.gain.linearRampToValueAtTime(pk, ta + atk);
      if (kind === 'hiss') { g.gain.setValueAtTime(pk, ta + atk + dur * 0.55); g.gain.setTargetAtTime(0, ta + atk + dur * 0.55, dur * 0.15); }
      else g.gain.setTargetAtTime(0, ta + atk, dur / 3);
      chain(s, hp, bp, lp, g, v.out);
      v.src(s, ta, end + dur * 1.4 + 0.02);
      var s2 = noiseSrc(PINK), bp2 = v.n(F('bandpass', rand(600, 850), 1.1)), g2 = v.n(G(0));
      g2.gain.setValueAtTime(0, ta);
      g2.gain.linearRampToValueAtTime(pk * 0.6, ta + atk);
      g2.gain.setTargetAtTime(0, ta + atk, dur / (kind === 'hiss' ? 2 : 6));
      chain(s2, bp2, g2, v.out);
      v.src(s2, ta, end + dur * 1.4 + 0.02);
      roomInto(v, num(o.room, 0.1), 3000);
      v.commit();
    };

    // Precision robotic-arm glide: an S-curve move (accelerate ~30 %, cruise, decelerate ~30 %). The motor
    // tone (triangle + a quiet octave) follows the speed from `from` to `to` (MIDI; `to` folds above A4:
    // it is sustained), a stepper "zipper" (the edges of a sawtooth at 35–255 steps/s, high-passed) is
    // clearest while slow, a narrow band of gear whine and a little air follow the speed, and a soft brake
    // click lands at the end. { dur (0.9 s, ≤ 4), to (57 = A3), from (to − 14), settle (true), room (0.1),
    //   gain, pan, when }
    P.arm = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.13, reverb: 0.04, delay: 0 });
      var t = v.t, dur = clamp(num(o.dur, 0.9), 0.15, 4);
      var mTo = foldIf(num(o.to, 57), o, CLOUD_MAX), mFrom = Math.min(num(o.from, mTo - 14), mTo);
      var fLo = midiToHz(mFrom), fHi = midiToHz(mTo), N = 64, acc = rand(0.26, 0.34), dec = rand(0.26, 0.34);
      var fC = new Float32Array(N), f2C = new Float32Array(N), wC = new Float32Array(N), sC = new Float32Array(N);
      var aC = new Float32Array(N), stC = new Float32Array(N), wgC = new Float32Array(N), airC = new Float32Array(N);
      for (var i = 0; i < N; i++) {
        var x = i / (N - 1), sp = x < acc ? sstep(x / acc) : x > 1 - dec ? sstep((1 - x) / dec) : 1;
        var f = fLo * Math.pow(fHi / fLo, sp);
        fC[i] = f; f2C[i] = f * 2.003; wC[i] = Math.min(f * 4.1, NYQ);
        sC[i] = 35 + 220 * sp;
        aC[i] = Math.sqrt(sp);
        stC[i] = 0.07 * Math.min(1, sp * 6) * (0.4 + 0.6 * (1 - sp));
        wgC[i] = 0.05 * sp; airC[i] = 0.08 * sp;
      }
      var mot = O('triangle', fC[0]), oct = O('sine', f2C[0]);
      mot.frequency.setValueCurveAtTime(fC, t, dur); oct.frequency.setValueCurveAtTime(f2C, t, dur);
      var ma = v.n(G(0)), go = v.n(G(0.16)), mlp = v.n(F('lowpass', 1500, 0.7)), mhp = v.n(F('highpass', 90, 0.7));
      ma.gain.setValueCurveAtTime(aC, t, dur);
      mot.connect(ma); oct.connect(go); go.connect(ma);
      chain(ma, mlp, mhp, v.out);
      var st = O('sawtooth', sC[0]), sa = v.n(G(0)), shp = v.n(F('highpass', 1800, 0.7)), slp = v.n(F('lowpass', 4200, 0.7));
      st.frequency.setValueCurveAtTime(sC, t, dur); sa.gain.setValueCurveAtTime(stC, t, dur);
      chain(st, shp, slp, sa, v.out);
      var wn = noiseSrc(WHITE), wbp = v.n(F('bandpass', wC[0], 9)), wa = v.n(G(0));
      wbp.frequency.setValueCurveAtTime(wC, t, dur); wa.gain.setValueCurveAtTime(wgC, t, dur);
      chain(wn, wbp, wa, v.out);
      var an = noiseSrc(PINK), abp = v.n(F('bandpass', rand(420, 600), 0.8)), aa = v.n(G(0));
      aa.gain.setValueCurveAtTime(airC, t, dur);
      chain(an, abp, aa, v.out);
      var stop = t + dur + 0.02;
      v.src(mot, t, stop); v.src(oct, t, stop); v.src(st, t, stop); v.src(wn, t, stop); v.src(an, t, stop);
      if (o.settle !== false) modesInto(v, v.out, t + dur - 0.004, [[rand(900, 1200), 0.004, 0.4], [rand(240, 300), 0.012, 0.55]], 0.3);
      roomInto(v, num(o.room, 0.1), 3000);
      v.commit();
    };

    // Metallic snap-fit / latch: a bright, short modal click — a free bar's inharmonic partials
    // (1 : 2.76 : 5.40) on a 1.0–1.76 kHz fundamental with a tiny ring, a plastic housing knock and a
    // sub-millisecond contact — then, 6–16 ms later, the latch seating (quieter, a little higher).
    // { size 0..1 (0.4: lower, longer ring), metal 0..1 (0.6: ring and upper partials), double (true),
    //   room (0.14), gain, pan, when }
    P.snap = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.105, pan: rand(-0.3, 0.3), reverb: 0.04, delay: 0.02 });
      var t = v.t, size = clamp(num(o.size, 0.4), 0, 1), metal = clamp(num(o.metal, 0.6), 0, 1);
      function hit(tt, k, fk) {
        var f0 = Math.min(1760, (1650 - 650 * size) * rand(0.94, 1.06) * fk), ring = (0.01 + 0.02 * size) * (0.5 + metal);
        modesInto(v, v.out, tt, [
          [f0, ring, 1],
          [f0 * 2.756 * rand(0.985, 1.015), ring * 0.42, 0.2 + 0.4 * metal],
          [f0 * 5.404 * rand(0.985, 1.015), ring * 0.2, 0.28 * metal],
          [rand(420, 560) * (1 - 0.25 * size), 0.007, 0.6],
          [rand(900, 1150), 0.003, 0.35]
        ], k);
        noiseTick(v, v.out, tt, rand(0.0004, 0.0008), 2000, 6500, 0.35 * k);
      }
      hit(t, 1, 1);
      if (o.double !== false) hit(t + rand(0.006, 0.016), rand(0.35, 0.55), rand(1.03, 1.1));
      roomInto(v, num(o.room, 0.14), 3800);
      v.commit();
    };

    // Relay snap: the armature slamming onto the core — a dry 2–3 kHz click on a small metal body and a
    // 170–240 Hz knock of the case on its board — with 2–4 contact bounces inside ~3 ms and a faint
    // leaf-spring ring. kind 'off' = the release: lighter, hardly any knock, no bounce.
    // { kind: 'on' (default) | 'off', size 0..1 (0.4: bigger = lower), room (0.12), gain, pan, when }
    P.relay = function (o) {
      o = o || {};
      var off = o.kind === 'off', size = clamp(num(o.size, 0.4), 0, 1), lo = 1 - 0.25 * size;
      var v = Voice(o, { level: 0.095, pan: rand(-0.3, 0.3), reverb: 0.02, delay: 0.02 });
      var t = v.t, a = off ? 0.55 : 1;
      modesInto(v, v.out, t, [
        [rand(2100, 2900) * lo, 0.0011, 1],
        [rand(900, 1200) * lo, 0.003, 0.55],
        [rand(170, 240) * lo, 0.009, off ? 0.1 : 0.55],
        [rand(3300, 4100), 0.006, 0.07]
      ], a);
      noiseTick(v, v.out, t, rand(0.0003, 0.0006), 1800, 6000, 0.3 * a);
      if (!off) {
        var nb = 2 + Math.floor(Math.random() * 3), tb = t, ab = 0.45;
        for (var i = 0; i < nb; i++) {
          tb += rand(0.0004, 0.0011) * Math.pow(1.35, i);
          clickP(v, v.out, tb, rand(2600, 3600) * lo, rand(2.5, 4), ab * rand(0.8, 1.1));
          ab *= rand(0.45, 0.6);
        }
      }
      roomInto(v, num(o.room, 0.12), 3600);
      v.commit();
    };
    ...
    // CNC / machining hum (round 4, subtle): a spindle tone (two triangles ~4 cents apart + its octave and a faint
    // 3rd, with a slow wander and a 6 Hz shimmer of a few cents, so it never reads as a pure whistle) under
    // a lowpass that opens with load, a spindle-air band, and — with load — cutting noise chopped at the
    // tool's chatter rate plus a low chip rumble. Sustained: the spindle pitch folds above A4.
    // { rate 0..1 (0.7: spindle speed; 0 = spun down, tone silent), load 0..1 (0.3: cutting), midi (57 = A3,
    //   the full-speed spindle pitch), gain, attack (1.2), dest, reverb }; set({ rate, load, gain, midi }, ramp)
    E.cnc = function (o) {
      o = o || {};
      var rate = clamp(num(o.rate, 0.7), 0, 1), load = clamp(num(o.load, 0.3), 0, 1), baseM = fold(num(o.midi, 57), CLOUD_MAX);
      var c = Continuous({ dest: o.dest, when: o.when, reverb: o.reverb, defReverb: 0.04 }, 0.1 * Math.max(0, num(o.gain, 1)));
      function fS() { return midiToHz(baseM) * (0.42 + 0.58 * rate) * Math.pow(2, -0.7 * load / 12); }
      function toneLv() { return sstep(rate / 0.25) * 0.55; }
      var f = fS();
      var t1 = O('triangle', f, -2), t2 = O('triangle', f, 2.5), h2 = O('sine', f * 2, 2), h3 = O('triangle', f * 3, -3);
      var wob = O('sine', rand(5.6, 6.8)), wobG = c.n(G(3)), wan = O('sine', rand(0.19, 0.31)), wanG = c.n(G(6));
      wob.connect(wobG); wan.connect(wanG);
      [t1, t2, h2, h3].forEach(function (x) { wobG.connect(x.detune); wanG.connect(x.detune); });
      var tg = c.n(G(toneLv())), g1 = c.n(G(0.62)), g2 = c.n(G(0.26)), gh2 = c.n(G(0.2)), gh3 = c.n(G(0.06));
      var tlp = c.n(F('lowpass', 650 + 700 * load, 0.8)), thp = c.n(F('highpass', 70, 0.7));
      t1.connect(g1); t2.connect(g2); h2.connect(gh2); h3.connect(gh3);
      [g1, g2, gh2, gh3].forEach(function (g) { g.connect(tlp); });
      chain(tlp, thp, tg, c.out);
      var an = noiseSrc(PINK), abp = c.n(F('bandpass', f * 4, 0.7)), ag = c.n(G(0.12 * rate));
      chain(an, abp, ag, c.out);
      var cn = noiseSrc(WHITE), cbp = c.n(F('bandpass', 1500 + 600 * load, 0.9)), clp = c.n(F('lowpass', 3500, 0.6));
      var chop = O('sawtooth', f * 0.25), chopD = c.n(G(0.35)), cam = c.n(G(0.65)), cg = c.n(G(0.3 * load));
      chop.connect(chopD); chopD.connect(cam.gain);
      chain(cn, cbp, clp, cam, cg, c.out);
      var rn = noiseSrc(PINK), rlp = c.n(F('lowpass', 260, 0.7)), rg = c.n(G(0.25 * load));
      chain(rn, rlp, rg, c.out);
      [t1, t2, h2, h3, wob, wan, an, cn, chop, rn].forEach(c.src);
      c.fadeIn(Math.max(0.05, num(o.attack, 1.2)));
      return {
        set: function (p, ramp) {
          if (c.stopped || !p) return;
          var r = Math.max(0, num(ramp, 0.5));
          if (isFinite(p.rate)) rate = clamp(p.rate, 0, 1);
          if (isFinite(p.load)) load = clamp(p.load, 0, 1);
          if (isFinite(p.midi)) baseM = fold(p.midi, CLOUD_MAX);
          if (isFinite(p.gain)) { c.level = 0.1 * Math.max(0, p.gain); glideTo(c.out.gain, c.level, r); }
          var nf = fS();
          glideTo(t1.frequency, nf, r); glideTo(t2.frequency, nf, r); glideTo(h2.frequency, nf * 2, r); glideTo(h3.frequency, nf * 3, r);
          glideTo(chop.frequency, nf * 0.25, r); glideTo(abp.frequency, nf * 4, r);
          glideTo(tg.gain, toneLv(), r); glideTo(ag.gain, 0.12 * rate, r);
          glideTo(tlp.frequency, 650 + 700 * load, r); glideTo(cbp.frequency, 1500 + 600 * load, r);
          glideTo(cg.gain, 0.3 * load, r); glideTo(rg.gain, 0.25 * load, r);
        },
        stop: function (rel) { c.stop(num(rel, 1.2)); }
      };
    };

    // Server-fan whir (round 4, a bed): two fans (L / R, 1.2 % apart, so they slowly beat), each one pink-noise
    // source into broadband air (low-passed) and the blade-pass whir — a narrow band-pass at the blade-pass
    // frequency and its octave (tonal noise, never a pure sine; 160–380 Hz, below A4) — with a very slow drift.
    // { speed 0..1 (0.6; `rate` is accepted too), gain, attack (2), dest, reverb }; set({ speed | rate, gain }, ramp)
    E.fan = function (o) {
      o = o || {};
      var sp = clamp(num(o.speed, num(o.rate, 0.6)), 0, 1), gm = Math.max(0, num(o.gain, 1));
      function lvl() { return (0.35 + 0.65 * sp) * 0.3 * gm; }
      function bpf() { return 160 + 220 * sp; }
      var c = Continuous({ dest: o.dest, when: o.when, reverb: o.reverb, defReverb: 0.03 }, lvl());
      var fans = [[-0.55, 1], [0.55, 1.012]].map(function (cfg) {
        var s = noiseSrc(PINK), fg = c.n(G(1)), p = c.n(PAN(cfg[0]));
        var b1 = c.n(F('bandpass', bpf() * cfg[1], 10)), g1 = c.n(G(1.6));
        var b2 = c.n(F('bandpass', 2 * bpf() * cfg[1], 12)), g2 = c.n(G(0.3));
        var l3 = c.n(F('lowpass', 900 + 1300 * sp, 0.5)), h3 = c.n(F('highpass', 130, 0.7)), g3 = c.n(G(0.3));
        chain(s, b1, g1, fg); s.connect(b2); chain(b2, g2, fg); s.connect(l3); chain(l3, h3, g3, fg);
        var lf = O('sine', rand(0.07, 0.13)), lg = c.n(G(0.08));
        lf.connect(lg); lg.connect(fg.gain);
        chain(fg, p, c.out);
        [s, lf].forEach(c.src);
        return { b1: b1, b2: b2, l3: l3, k: cfg[1] };
      });
      c.fadeIn(Math.max(0.05, num(o.attack, 2)));
      return {
        set: function (p, ramp) {
          if (c.stopped || !p) return;
          var r = Math.max(0, num(ramp, 1));
          var ns = isFinite(p.speed) ? p.speed : p.rate;
          if (isFinite(ns)) sp = clamp(ns, 0, 1);
          if (isFinite(p.gain)) gm = Math.max(0, p.gain);
          c.level = lvl();
          glideTo(c.out.gain, c.level, r);
          fans.forEach(function (fn) {
            glideTo(fn.b1.frequency, bpf() * fn.k, r); glideTo(fn.b2.frequency, 2 * bpf() * fn.k, r);
            glideTo(fn.l3.frequency, 900 + 1300 * sp, r);
          });
        },
        stop: function (rel) { c.stop(num(rel, 2)); }
      };
  ...
  ['click', 'tick', 'bit', 'data', 'grain', 'glitch', 'ratchet', 'sub',
    ...
    // round 4 micro-foley ("Bits & Atoms")
  ...
  function deferredSource(kind, args) {
    ...
    function realize() {
      ...
      try {
        var a = args.slice(), oi = kind === 'drone' || kind === 'hum' ? 1 : 0, o = {}, k;
        var src = a[oi] || {};
  ...
  A.cnc = function (o) { return deferredSource('cnc', [o]); };   // round 4: CNC / machining hum
  A.fan = function (o) { return deferredSource('fan', [o]); };   // round 4: server-fan whir
  ...
  /* ---------------------------------------------------------------- Bits & Atoms phrase (round 4) */
  // Site.audio.assemblyPhrase(o?) → { duration, marks: [{ t, label }], events: [[t, fn(X, when, dest, st)]], release(st, fade) }
  // The audition phrase for the micro-foley family (audio-lab.html plays it live, audio-check.html renders it
  // offline), and a worked example of the Build-page arc: one keystroke and one servo actuation in silence →
  // a hesitant, human "prompt" being typed, answered by a relay, a snap-fit and an arm → the same sounds
  // quantized and layered while the tempo accelerates (16th: 220 → 90 ms) into a dense, syncopated "assembly
  // engine" over a swelling sub, fan and CNC hum → a compile-success chime and a warm pad, crossfading into a
  // chill low granular bed. X = Site.audio or a raw engine (both have play.*, cnc(), fan(), granular());
  // dest = a bus (null → master); st = an object the events share (continuous handles). o.seed fixes the pattern.
  A.assemblyPhrase = function (o) {
    o = o || {};
    var seed = (Math.abs(Math.round(num(o.seed, 7))) >>> 0) || 7;
    ...
    var ev = [], marks = [];
    function mark(t, label) { marks.push({ t: t, label: label }); }
    function opts(base, w, d) { var r = {}, k; for (k in base) r[k] = base[k]; r.when = w; if (d) r.dest = d; return r; }
    // hit(t, 'key', {…}) · hit(t, 'chime', 74, {…}) · hit(t, 'pad', [50, 57], {…})
    function hit(t, name, a, b) {
      var note = typeof a === 'number' || Array.isArray(a), ob = note ? b || {} : a || {};
      ev.push([t, function (X, w, d) {
        var p = X && X.play && X.play[name];
        if (typeof p === 'function') { if (note) p(a, opts(ob, w, d)); else p(opts(ob, w, d)); }
      }]);
    }
    function at(t, fn) { ev.push([t, fn]); }

    // 1) silence: one keystroke, one servo actuation
    mark(0, 'one keystroke · one servo');
    hit(0.25, 'key', { vel: 0.72, up: 0.35 });
    hit(1.1, 'servo', { from: 57, to: 64, dur: 0.32, load: 0.25, settle: 1 });

    // 2) a prompt typed by hand: irregular, hesitant at first, one correction; the fan comes up
    mark(2.4, 'a prompt, typed by hand');
    at(2.4, function (X, w, d, st) { if (X.fan) st.fan = X.fan({ speed: 0.3, gain: 0.2, attack: 2.5, dest: d, when: w }); });
    var tt = 2.45, word = 0, wl = 3 + Math.floor(rr(0, 3)), typed = 0;
    while (tt < 5.0) {
      if (typed === 9) {
        hit(tt, 'key', { kind: 'backspace', vel: 0.8, up: 0.3 }); tt += rr(0.16, 0.22);
        hit(tt, 'key', { kind: 'backspace', vel: 0.75, up: 0.3 }); tt += rr(0.22, 0.3);
        typed++; continue;
      }
      if (word >= wl) {
        hit(tt, 'key', { kind: 'space', vel: rr(0.6, 0.8), up: 0.35 });
        word = 0; wl = 2 + Math.floor(rr(0, 4)); tt += rr(0.2, 0.34); continue;
      }
      hit(tt, 'key', { vel: rr(0.45, 0.8), tone: rr(0.25, 0.45), up: rr(0.2, 0.4) });
      word++; typed++;
      tt += rr(0.11, 0.26) * (typed < 4 ? 1.5 : 1);
    }
    var tE = Math.max(tt, 5.05);
    hit(tE, 'key', { kind: 'enter', vel: 0.95, up: 0.4 });
    hit(tE + 0.14, 'relay', { size: 0.35 });
    hit(tE + 0.2, 'compile', { count: 5, gap: 0.045, progress: 0.05 });
    hit(tE + 0.3, 'arm', { dur: 1.1, from: 48, to: 57, gain: 0.3 });
    hit(tE + 0.55, 'snap', { size: 0.4 });
    hit(tE + 0.62, 'pneumatic', { kind: 'puff', pressure: 0.4 });

    // 3) the same sounds on a grid, layering in while the tempo accelerates, then 4) one bar at full speed
    var tC = tE + 0.9, N = 48, sd0 = 0.22, sd1 = 0.09, r = Math.pow(sd1 / sd0, 1 / (N - 1));
    var times = [], tx = tC, s;
    for (s = 0; s < N + 16; s++) { times.push(tx); tx += s < N ? sd0 * Math.pow(r, s) : sd1; }
    var tD = times[N], tEnd = tx;
    mark(tC, 'quantized · accelerating');
    mark(tD, 'assembly engine');
    at(tC, function (X, w, d, st) {
      if (X.cnc) st.cnc = X.cnc({ rate: 0.3, load: 0, gain: 0.22, attack: 2, dest: d, when: w });
      if (st.fan) st.fan.set({ speed: 0.45, gain: 0.24 }, 2);
    });
    function machines(p) {
      return function (X, w, d, st) {
        if (st.cnc) st.cnc.set({ rate: 0.35 + 0.65 * p, load: 0.1 + 0.6 * p, gain: 0.22 + 0.26 * p }, 1.2);
        if (st.fan) st.fan.set({ speed: 0.45 + 0.45 * p, gain: 0.24 + 0.12 * p }, 1.2);
      };
    }
    var RIM = euclid(5, 16, 3), RIM7 = euclid(7, 16, 1), SNP = euclid(3, 8, 1);
    var tPad = times[Math.round(N * 0.4)];
    hit(tPad, 'pad', [50, 57, 62, 64], { dur: tEnd - tPad, attack: (tD - tPad) * 0.9, release: 0.5, cutoff: 900, grains: 0, gain: 0.26 });
    hit(tD, 'noise', { dur: tEnd - tD - 0.05, attack: tEnd - tD - 0.05, release: 0.04, filter: 'bandpass', freq: 500, freqTo: 2600, q: 1.2, gain: 0.06 });
    for (s = 0; s < N + 16; s++) {
      var T = times[s], sd = s < N ? sd0 * Math.pow(r, s) : sd1, p = Math.min(1, s / (N - 1)), peak = s >= N, i16 = s % 16, bar = Math.floor(s / 16);
      var hj = peak ? 0 : (1 - p) * 0.012, j = rr(-hj, hj); // the human timing melts into the grid
      var pg = peak ? 1.25 : 1;                             // the engine hits a little harder
      if (i16 === 0) at(T, machines(peak ? 1 : p));
      // keys: 8ths, then 16ths (the hats); the space bar takes the backbeat (the snare) once the groove forms
      var back = i16 === 4 || i16 === 12;
      if (back) hit(T + j, 'key', p > 0.15 ? { kind: 'space', vel: 0.9, up: 0.25, hold: sd * 1.1, gain: pg } : { vel: 0.8 });
      else if (rnd() < (p < 0.3 ? (i16 % 2 === 0 ? 0.85 : 0) : 0.55 + 0.45 * p)) {
        hit(T + j, 'key', { vel: i16 % 4 === 0 ? 0.85 : rr(0.4, 0.62), tone: 0.3 + 0.35 * p, up: 0.25 * (1 - 0.6 * p), hold: sd * rr(0.45, 0.6), gain: pg });
      }
      if (peak && i16 === 14 && rnd() < 0.6) hit(T, 'key', { kind: 'space', vel: 0.45, up: 0 });
      if (i16 === 10 && p > 0.5 && rnd() < 0.6) hit(T, 'key', { kind: 'backspace', vel: 0.75, up: 0.2 });
      if (i16 === 15 && p > 0.65) hit(T, 'key', { kind: 'enter', vel: 0.85, up: 0.2, hold: sd * 0.8 });
      // relays as the rim (euclidean 5 → 7 of 16), snap-fits on a 3-3-2
      if ((peak ? RIM7 : RIM)[i16] && p > 0.28) hit(T, 'relay', { kind: s % 3 === 0 ? 'off' : 'on', size: 0.35, gain: 0.85 * pg });
      if (SNP[s % 8] && p > 0.45) hit(T, 'snap', { size: 0.25 + 0.25 * rnd(), metal: 0.5 + 0.3 * p, gain: 0.75 * pg });
      // servos: every bar (climbing), then every half bar the other way
      if (i16 === 0 && s > 0) hit(T, 'servo', { from: 55 + bar * 2, to: 62 + bar * 2, dur: sd * 5, load: 0.2 + 0.5 * p, gain: 0.8 });
      if (i16 === 8 && p > 0.6) hit(T, 'servo', { from: 66, to: 59, dur: sd * 3.5, load: 0.5, gain: 0.7 });
      // air: puffs on the "and" of beat 2, a release before the engine
      if (i16 === 7 && p > 0.35) hit(T, 'pneumatic', { kind: 'puff', pressure: 0.4 + 0.3 * p, gain: 0.7 });
      if (s === N - 4) hit(T, 'pneumatic', { kind: 'release', dur: 0.4, pressure: 0.7, gain: 0.8 });
      // compile ticks thicken into offbeat 32nds, climbing
      if (p > 0.55 && (peak || (i16 % 2 === 1 && rnd() < (p - 0.55) * 2.2))) {
        hit(T + sd * 0.5, 'compile', { progress: peak ? 0.55 + (0.45 * (s - N)) / 15 : p * 0.6, gain: 0.8 });
      }
      // arm glides; the sub swells in under the second half and pulses on every beat of the engine
      if (s === 0) hit(T, 'arm', { dur: 2.2, from: 48, to: 57, gain: 0.22 });
      if (s === 32) hit(T, 'arm', { dur: 1.3, from: 50, to: 60, gain: 0.22 });
      if (s === N) hit(T, 'arm', { dur: sd1 * 14, from: 52, to: 64, gain: 0.22 });
      if (!peak && p > 0.45 && i16 % 8 === 0) hit(T, 'sub', 38, { dur: sd * 3, gain: 0.08 + 0.1 * p });
      if (peak && i16 % 4 === 0) hit(T, 'sub', 38, { dur: sd * 2.2, gain: 0.12 });
    }

    // 5) resolution: the latch closes, "compile success", a warm pad; the machines crossfade into the chill bed
    var tR = tEnd + sd1 * 0.5;
    mark(tR, 'compile success');
    hit(tR, 'relay', { size: 0.5 });
    hit(tR + 0.004, 'snap', { size: 0.5, metal: 0.4, gain: 0.8 });
    hit(tR + 0.02, 'chime', 69, { dur: 1.0, vel: 0.6, gain: 0.7 });
    hit(tR + 0.095, 'chime', 74, { dur: 1.0, vel: 0.62, gain: 0.7 });
    hit(tR + 0.17, 'chime', 78, { dur: 1.1, vel: 0.66, gain: 0.7 });
    hit(tR + 0.32, 'chime', 81, { dur: 1.2, vel: 0.75, bright: 0.35, gain: 0.7 });
    hit(tR + 0.02, 'pad', [50, 57, 62, 66, 69], { dur: 1.6, attack: 0.35, release: 1.5, cutoff: 1400, grains: 5, bright: 0.2, gain: 0.24 });
    hit(tR + 0.02, 'sub', 38, { dur: 1.2, gain: 0.1 });
    at(tR, function (X, w, d, st) {
      if (st.cnc) st.cnc.stop(1.1);
      if (st.fan) st.fan.stop(1.8);
    });
    at(tR + 0.6, function (X, w, d, st) {
      if (X.granular) st.bed = X.granular({ density: 5, pitch: [50, 54, 57, 62, 66, 69], dur: 0.16, window: 'tukey', spread: 0.8, bright: 0.12, octave: 0, detune: 4, gain: 0.28, attack: 2.2, highpass: 110, dest: d, when: w });
    });
    mark(tR + 1.4, 'chill bed');
    at(tR + 4.6, function (X, w, d, st) { if (st.bed) st.bed.stop(2.4); });
    ...
    return {
      ...
      events: ev,
      resolveAt: tR,
      release: function (st, fade) {
        if (!st) return;
        ['cnc', 'fan', 'bed'].forEach(function (k) { if (st[k]) { try { st[k].stop(num(fade, 0.4)); } catch (e) {} st[k] = null; } });
      }
    };
  };
