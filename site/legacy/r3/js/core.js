...
(function () {
  ...
  function layoutMobileStack() {
    ...
    if (innerWidth > 800) { ui.controls.style.bottom = ''; return; }
    const capTop = 58 + ui.caption.offsetHeight;
    ui.controls.style.bottom = Math.max(104, capTop + 16) + 'px';
  }
  ...
  function createInstance(def) {
    ...
    const rec = {
      def, el, controls, links,
      loops: new Set(), resizers: new Set(), canvases: [], placers: [],
      caption: def.caption || '',
      hooks: {}, soundOn: false,
    };
  ...
  /* ------------------------------------------------------------------ wordmark: Yash Dagade ⇄ Energy and Intelligence */

  const WORDMARK = ['Yash Dagade', 'Energy and Intelligence'];
