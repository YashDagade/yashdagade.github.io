/* ============================================================================
   yashdagade.com v2 — scene [3] · Rectified LpJEPA   (id 'lpjepa')
   ...
   Layout is responsive per layer: phones (< 800 px) stack everything; narrow desktop art boxes
   stack the dial over its panel (step 2–3), move legends out of plots (step 4) and drop the
   side tables for a one-line Table 5 readout (step 5). Shared header style: every step's kicker starts on the
   content-left (300 px ≥ 1280, 280 px ≥ 1100, else 260) at y ≈ 86, and the step counter sits top-right; art that
   reaches the counter's box moves left of it (step 1 ball, step 4 matrix) or under it (steps 2–3 panel, step 5 tables).
...
(function () {
  ...
  const CAPS = [
    'A rectified generalized Gaussian:\nall negative mass collapses into\none exact point mass at zero.',
    'Cramér–Wold: two laws are equal\niff every 1-D projection is.\nDrag the line to try a direction.',
    'Training sorts random 1-D slices\nand matches them rank by rank.\nPoints collapse onto the axes.',
    'The point mass is a dial: the\nmean shift sets the share of\nactive neurons, in closed form.',
    'Most of the code can be exactly\nzero before accuracy moves:\nencoder −3.8 pts at 97.8% zeros.',
  ];
  const DUR = [13, 15, 17, 14, 13];      // autoplay seconds per step
  ...
  Site.register({
    id: 'lpjepa', n: 3, title: 'Rectified LpJEPA', path: '/lpjepa',
    caption: CAPS[0],
    create(el, api) {
      ...
      const RE_LEG_M = memo(() => `<div class="lpj-grid">${legRow(SW.ghost, '\\mathrm{GN}_p', '')}${legRow(SW.solid, '\\mathrm{RGN}_p', '')}${legRow(SW.dash, '\\mathrm{TGN}_p', '')}${legRow(SW.mass, '\\Phi_p(-\\mu/\\sigma)', '')}</div>`);
      ...
      function reHeadHTML() {
        ...
        return `<div class="kick">RECTIFIED GENERALIZED GAUSSIAN · DEF. 3.4</div>`
          + `<div style="margin:5px 0 3px">${K('\\mathrm{RGN}_p(\\mu,\\sigma)', mob ? 22 : 28)}</div>`
          + `<div>${K(settled ? `p = ${fmtP(p)}` : `p \\to ${fmtP(S.p)}`)}${name && !mob ? `<span class="dim"> ${name}</span>` : ''}<span class="dim"> · </span>${K('\\mu =')}<span class="ink"> ${fmt(S.mu, 2)}</span><span class="dim"> · </span>${K(mob ? '\\sigma =' : '\\sigma = \\sigma_{\\mathrm{GN}}(p) =')}<span class="ink"> ${sigmaGN(p).toFixed(3)}</span></div>`;
      }
      // Cramér–Wold header, widest first: the statement breaks only at ⟺ and the legend rows never wrap,
      // so narrow boxes get shorter words instead of a line broken mid-equation (layout picks the first that fits)
      const CW_V = [], CW_L2 = '\\Longleftrightarrow\\;\\mathrm{law}(c^{\\top}z)=\\mathrm{law}(c^{\\top}y)';
      function cwHeadHTML(v) {
        if (CW_V[v]) return CW_V[v];
        const L1 = '\\mathrm{law}(z)=\\mathrm{law}(y)', L2 = '\\mathrm{law}(c^{\\top}z)=\\mathrm{law}(c^{\\top}y)';
        if (v === 4) {
          return (CW_V[v] = `<div class="kick">CRAMÉR–WOLD DEVICE · EQ. 13</div>`
            + `<div style="margin:7px 0 8px"><div class="lpj-nw">${K(L1, 18)}</div><div class="lpj-nw" style="margin-top:2px">${K(CW_L2 + '\\ \\forall c', 18)}</div></div>`
            + CW_HEAD_M());
        }
        const every = v >= 3 ? '\\ \\text{ for all } c' : '\\ \\text{ for every direction } c';
        const stmt = v === 0
          ? `<div class="lpj-nw">${K(`${L1}\\;\\Longleftrightarrow\\;${L2}${every}`, 18)}</div>`
          : `<div class="lpj-nw">${K(`${L1}`, 18)}</div><div class="lpj-nw" style="margin-top:2px">${K(`\\Longleftrightarrow\\;${L2}${every}`, 18)}</div>`;
        const short = v >= 2;
        const row = (sw, tex, words) => `<div class="lpj-row lpj-nw">${sw}${K(tex)}<span class="dim">${words}</span></div>`;
        return (CW_V[v] = `<div class="kick">CRAMÉR–WOLD DEVICE · EQ. 13</div>`
          + `<div style="margin:7px 0 8px">${stmt}</div>`
          + row(SW.accDot, 'y\\sim\\mathrm{RGN}_p(\\mu,\\sigma)', short ? 'target' : 'target samples')
          + row(SW.inkDot, 'z=\\mathrm{ReLU}(Wx+b),\\ x\\sim\\mathrm{GN}_p(0,I)', short ? 'encoder' : 'toy encoder')
          + row(SW.band, 'W_2^2(\\theta)', short ? 'all zero: same law' : 'per direction · all zero means same law'));
      }
      ...
      function t5HTML(p, w) {
        ...
        s += '</table>';
        s += `<div class="wrap dim" style="width:${w}px;margin-top:8px">Lower ${K('p', 14)}: sparser at the same ${K('\\mu', 14)}, accuracy flat.</div>`;
        return s;
      }
      ...
      const L = { ready: false, dirty: [true, true, true, true], mob: false, w: 0, h: 0 };
      ...
      function layout() {
        ...
        // shared content-left (every step's kicker/title starts here): 300 px on wide screens, 260 px below 1280;
        ...
        const y0 = mob ? 114 : 78;
        const y1 = mob ? H - 196 : H - 92;
        L.art = { x0, x1, y0, y1, w: x1 - x0, h: y1 - y0, cx: (x0 + x1) / 2, cy: (y0 + y1) / 2 };
        // the step counter's box (top-right, 3 rows from y = 72): art that reaches it moves left of it or below it.
        // It always reads "03 / 05 · Slice & train", like every other scene's counter.
        ...
        if (mob) L.cb = null;
        else L.cb = { x0: w - 40 - Math.ceil(Math.max(lw, tw('paused · [space]'))) - 14, y1: 72 + 42 + 8 };
        ...
        if (mob) for (const t of tags) posTag(t, x1, 90, 1, 0.5); // beside the step nav, right-aligned
      }
      const LAYOUTS = [layRe, laySl, layDi, layPa];
      function ensure(l) { if (L.ready && L.dirty[l]) { L.dirty[l] = false; LAYOUTS[l](); } }

      // step 0 · density plot
      function layRe() {
        const A = L.art, mob = L.mob;
        prime('reHead', reHeadHTML());
        const hd = measure('reHead');
        const hh = hd.h || (mob ? 70 : 86);
        ...
        const top = A.y0;
        const clipY = top + hh + (mob ? 26 : 30); // the gap holds the "↑ peak" label of clipped densities
        const G = { x0: gx0, x1: gx0 + pw, w: pw, top, clipY, xmin: -3, xmax: 4, dmax: 1.2, legX0: Infinity, legY1: 0, showBall: false, legBelow: mob, massShort: mob, legKey: 'reLeg' };
        ...
        if (!mob) {
          ...
          // the unit ball sits top-right of the plot, left of the step counter
          const br = 36, by = top + 44, bx = Math.min(G.x1 - 44, L.cb.x0 - 12 - br - 8);
          G.ball = { bx, by, br };
          ...
          // definition + sampling rule wrap inside the plot width; the legend drops under them when it has no room above
          G.fSt = `max-width:${Math.round(pw)}px;white-space:normal`;
          prime('reFp', RE_RULE(), 'white-space:nowrap');
          G.fInline = (measure('reFp').w || 560) <= pw;
          prime('reF', RE_F(G.fInline), G.fSt);
          let yy = 64 + (measure('reF').h || 52) + 14;
          if (G.legBelow) { G.legDy = yy; yy += (lg.h || 88) + 12; }
          below = Math.max(196, yy + 40);
          G.tagDy = below - 38;
        ...
        G.base = Math.max(clipY + 140, A.y1 - below);
      ...
      function pickCwHead(maxW) {
        for (let v = 0; v < 4; v++) {
          prime('cwHead', cwHeadHTML(v), 'white-space:nowrap');
          if (measure('cwHead').w <= maxW) return { html: cwHeadHTML(v), st: `max-width:${Math.round(maxW)}px` };
        }
        ...
        if (measure('cwProbe').w <= maxW && maxW >= 200) return { html: cwHeadHTML(4), st: `width:${Math.round(maxW)}px;white-space:normal` };
        return { html: cwHeadHTML(3).replace(/ lpj-nw|class="lpj-nw"/g, ''), st: `max-width:${Math.round(maxW)}px;white-space:normal`, wrapped: true };
      }
      ...
      function laySl() {
        ...
        if (!mob && (A.w >= 600 || H < 800)) {
          ...
          // (left of the step counter) and the panel starts under it
          let ch = pickCwHead(P.x0 - A.x0 - 36), full = false;
          if (ch.wrapped) { full = true; ch = pickCwHead(Math.min(A.w, CB.x0 - 16 - A.x0)); }
          const headSt = ch.st;
          ...
          // table (146) and block (156) stack when there is room; otherwise they share one slot:
          // step 1 shows the table, step 2 the training block. The panel starts under the step counter.
          const FIX = 26 + 20 + 58, TBL = 146, BLK = 120, MINH = 2 * 46 + 32;
          let pTop = A.y0;
          if (P.x1 > CB.x0) pTop = Math.max(pTop, CB.y1);
          if (full) pTop = Math.max(pTop, A.y0 + (hd.h || 128) + 16);
          const avail = bottom - pTop;
          let rest = avail - FIX - TBL - BLK;
          P.stack = rest >= MINH; P.noTable = false; P.noBlock = false;
          if (!P.stack) {
            rest = avail - FIX - TBL; // one shared slot, sized for the taller table
            if (rest < MINH) { // too short for the table: the slot holds the training block (step 3) or "why sort" (step 2)
              P.noTable = true; rest = avail - FIX - BLK;
              if (rest < MINH) { P.noBlock = true; rest = avail - FIX; }
            }
          }
          P.noSlot = P.noTable && P.noBlock;
          const slotH = P.stack ? TBL + BLK : !P.noTable ? TBL : !P.noBlock ? BLK : 0;
          const hA = clamp(rest * 0.36, 46, 100), hL = clamp(rest * 0.2, 32, 62);
          const usedH = FIX + slotH + 2 * hA + hL;
          let y = pTop + Math.max(0, (avail - usedH) / 2);
          P.yTitle = y + 8; y += 26;
          ...
          L.sl = { cx, cy, R, U: R / 3.7, P, aura: aur, headKey: 'cwHead', headSt, headHTML: ch.html };
        } else {
          let headH, headKey, headSt, headHTML;
          if (mob) { headKey = 'cwHeadM'; headSt = `max-width:${Math.round(A.w)}px`; headHTML = CW_HEAD_M(); prime(headKey, headHTML, headSt); headH = measure(headKey).h || 22; }
          else { headKey = 'cwHead'; const ch = pickCwHead(Math.min(A.w, CB.x0 - 16 - A.x0)); headSt = ch.st; headHTML = ch.html; prime(headKey, headHTML, headSt); headH = measure(headKey).h || 150; }
          ...
          const top = A.y0 + headH + 10;
          const fit = extra => { const outer = (bottom - top - body - extra) / 2; return { outer, R: Math.min((A.w / 2 - 24) / 1.12, (outer - 12) / 1.12) }; };
          let slot = !mob, f = fit(slot ? 160 : 0);
          if (slot && f.R < 110) { slot = false; f = fit(0); }
          ...
          L.sl = { cx, cy, R, U: R / 3.7, P, aura: aur, headKey, headSt, headHTML };
        }
        // the figure tags ride on the kicker row (the bottom-left corner belongs to the caption)
        if (!mob) { const fx = A.x0 + kw('CRAMÉR–WOLD DEVICE · EQ. 13') + 22; posTag(tags[1], fx, A.y0 + 8, 0, 0.5); posTag(tags[2], fx, A.y0 + 8, 0, 0.5); }
      ...
      function layDi() {
        ...
        // fewer sample rows on short screens: the chart below is the point of this step
        const rows = mob ? 12 : H < 700 ? 8 : H < 800 ? 12 : 16;
        // desktop: the matrix starts on the content-left, under the kicker; it (and its per-row counts) ends left of
        // the step counter, whose box its top rows share
        const right = mob ? A.x1 : Math.min(A.x1 - 40, L.cb.x0 - 8);
        let cols = mob || A.w < 600 ? 32 : 64;
        let readW = mob ? 0 : 54;
        const fitP = rw => Math.min(mob ? 14 : 16, (mob ? A.w : right - A.x0 - rw) / cols, (A.h * 0.3) / rows);
        ...
        const G = { cols, rows, pitch, cell: Math.max(4, pitch - 3), gx0, gw, rowRead: readW > 0, showF: false, meanY: null, legIn: true, legShort: false, legSt: '', ghost: {} };
        ...
        let y = A.y0 + (mob ? 24 : 22); // desktop: the kicker (gy0 − 14) lands on the shared header line
        ...
        if (!mob && !G.rowRead) { G.meanY = y + 17; y += 26; }
        if (!mob) {
        ...
        L.di = G;
        if (!mob) posTag(tags[3], G.cx1, G.cy0 - 30, 1, 0.5);
      }
      ...
      function layPa() {
        ...
        const cw = bx1 - bx0;
        const G = { bx0, bx1, tableW, tables, tx: tables ? bx1 + 48 : Infinity, pline: !tables, legOut: !mob && cw < 480, thumbs: !mob && H >= 800 && cw >= 560 };
        G.by0 = A.y0 + (mob ? 46 : 44); // desktop: the kicker (by0 − 36) lands on the shared header line
        ...
        if (G.legOut) { G.legDy = bottom + 16; bottom += 70; }
        if (G.thumbs) { G.thDy = bottom + 20; bottom += 60; }
        G.by1 = A.y1 - bottom;
        const asp = H > 1.2 * L.w ? 1.4 : 1.05; // portrait tablets may use the height
        if (!mob && G.by1 - G.by0 > cw * asp) G.by1 = G.by0 + cw * asp; // cap the aspect; the chart hangs from the header
        const { by0, by1 } = G;
        ...
        G.legW = tw('dense: SimCLR, VICReg, LeJEPA');
      ...
      function goStep(i, user, force) {
        ...
        if (i === 1) {
          ...
          if (user !== 'silent' && !S.hinted) {
            S.hinted = true;
            clearTimeout(hintT);
            hintT = setTimeout(() => { if (api.isActive() && (S.step === 1 || S.step === 2)) { api.hint(L.mob ? 'Drag the line to rotate it' : 'Drag the line to rotate the projection', 3200); hintAt = performance.now(); } }, 900);
          }
        }
        if (i === 2) {
      ...
      function drawRect(st) {
        ...
        lab('reHead', reHeadHTML(), G.x0, G.top, { ay: 0 });
        ...
        if (!mob) {
          lab('reF', RE_F(G.fInline), G.x0, G.base + 64, { ay: 0, st: G.fSt });
          if (G.legBelow) lab('reLeg', RE_LEG(), G.x0, G.base + G.legDy, { ay: 0 });
        } else lab('reLegM', RE_LEG_M(), G.x0, G.base + 58, { ay: 0 });
      }
      ...
      function drawSlice() {
        ...
        lab(SL.headKey, SL.headHTML, L.art.x0, L.art.y0, { ay: 0, st: SL.headSt });
      ...
      function drawPanel(pa) {
        ...
        ctx.save();
        ctx.globalAlpha *= pa;
        lab('pjT', `<span class="kick">PROJECTION ONTO </span>${K('c')}`, P.x0, P.yTitle, { alpha: pa });
        ...
        function hist(hh, mass, yAx, dir, color, colorSoft) {
          ...
          const lbl = `zeros ${pct(mass)}`, lw = tw(lbl) + 6, span = lw / pxPerU;
        ...
        // series labels: right-aligned in the corner unless the histogram's right tail is tall there
        {
          const wl = 116 / pxPerU;
          const zRight = !busy(hZ, TB - wl, TB, P.hA - 16), yRight = !busy(hY, TB - wl, TB, P.hA - 16);
          lab('pjZ', `${K('c^{\\top}z')}<span> embeddings</span>`, zRight ? P.x1 : P.x0, P.yA - P.hA + 6, { ax: zRight ? 1 : 0, alpha: pa, color: '#000', cls: 'ko' });
          lab('pjY', `${K('c^{\\top}y')}<span> target</span>`, yRight ? P.x1 : P.x0, P.yB + P.hA - 6, { ax: yRight ? 1 : 0, alpha: pa, color: C.accent, cls: 'ko' });
        }
        ...
        if (!mob) { // the caption goes in the empty part of the gap (short form if needed), never on the links
          const rs = P.x1 - hi, ls = lo - P.x0, room = Math.max(rs, ls);
          const long = room >= L.sl.sortW + 14, txt = long ? 'sort, then pair rank by rank' : 'pair by rank';
          const need = (long ? L.sl.sortW : L.sl.sortW2) + 14;
          let side = S.sortSide || 'r';
          if (side === 'r' && rs < need && ls >= need) side = 'l';
          else if (side === 'l' && ls < need && rs >= need) side = 'r';
          S.sortSide = side;
          if ((side === 'r' ? rs : ls) >= need) lab('pjSort', txt, side === 'r' ? P.x1 : P.x0, (P.yA + P.yB) / 2, { ax: side === 'r' ? 1 : 0, alpha: pa, color: T3 });
        }
        ...
        if (mob) {
          lab('pjW', `${K('W_2^2(\\theta)')}<span> this slice</span>`, P.x0, P.yRead, { alpha: pa });
          ...
          text(zl, P.x0, P.yRead + 22, { color: T2 });
          ctx.restore();
          return;
        }
        ...
        if (!P.noTable && (P.stack || S.step !== 2)) {
          ...
          t += '</table>';
          t += `<div style="margin-top:8px">${K('q=\\Phi_p(-\\mu/\\sigma) =', 14)}<span class="ink"> ${q.toFixed(3)}</span>${L.sl.qLong ? `<span class="dim">  · interior </span>${K('(1-q)^D\\to 0', 14)}` : ''}</div>`;
      ...
      function drawDial() {
        ...
        const g = gg.cur;
        // feature matrix
        kick(G.kick, mob ? L.art.x0 : G.gx0, G.gy0 - 14); // phones centre the grid; the kicker stays on the shared left edge
        ...
        if (hovCell) {
          ...
          S.tip = { x: hc.x + cs + 10, y: hc.y - 8, html: `<div>sample ${hc.r + 1} · dim ${hc.c + 1}</div><div class="${hc.v > 0 ? 'acc' : 'dim'}">${hc.v > 0 ? 'value ' + hc.v.toFixed(3) : 'exact zero'}</div>` };
        }
        ...
        text('upper bound 1', G.cx1, G.cy0 - 9, { align: 'right', size: 11, color: TG });
        ...
        if (m) T5.p.forEach((pp, i) => { if (Math.hypot(m.x - x0c, m.y - Y_(T5.l0[i])) < (m.touch ? 12 : 7)) S.tip = { x: x0c + 12, y: Y_(T5.l0[i]) - 20, html: `<div>${K(`p = ${fmtP(pp)}`, 14)} · ${K('\\mu = 0', 14)} · CIFAR-100</div><div>active ${T5.l0[i].toFixed(4)} · theory 0.5000</div>` }; });
        if (hovPt) {
          const i = hovPt.i;
          S.tip = { x: hovPt.xx + 12, y: hovPt.yy - 24, html: `<div>${K(`\\mu = ${texNum(T13.mu[i])}`, 14)} · trained ViT-S, ${K('p = 1', 14)}</div><div>active ${T13.active[i].toFixed(4)} · theory ${activeFrac(1, T13.mu[i]).toFixed(4)}</div><div class="dim">trained features sit denser than theory at mid ${K('\\mu', 14)}</div>` };
        }
      ...
      function ghostLabs(G, X_, Y_, blkX1, blkY1) {
        ...
        const free = (x0, y0, x1, y1, own) => {
          if (x0 < G.cx0 + 4 || x1 > G.cx1 - 2 || y0 < G.cy0 + 4 || y1 > G.cy1 - 4) return false;
        ...
        P_OPTS.filter(pp => pp !== S.p).forEach((pp, j) => {
          ...
          // never near µ = 0, where all three curves cross (a label there would be ambiguous)
          const vs = [];
          for (let v = 0.95; v >= 0.3; v -= 0.05) vs.push(v);
          for (let v = -0.3; v >= -1.2; v -= 0.05) vs.push(v); // further left the curves merge
          let spot = null;
      ...
      function drawPareto(st) {
        ...
        text(G.ylab, G.bx0 + 8, G.by0 + 2, { color: T2 });
        ...
        // legend (in the lower-left of the plot, or under it on narrow charts)
        {
          const lx = G.legOut ? G.bx0 - 26 : G.bx0 + 12, ly = G.legOut ? G.by1 + G.legDy : G.by1 - (mob ? 44 : 52);
          if (!G.legOut) { ctx.fillStyle = C.bg; ctx.fillRect(lx - 4, ly - 10, G.legW + 26, 56); } // knockout: guides pass behind
          ...
          text(mob ? 'dense baselines, 0% zeros' : 'dense: SimCLR, VICReg, LeJEPA', lx + 16, ly + 36, { color: T2 });
        }
        // hover
        const m = S.mouse;
        if (m) {
          ...
          if (best && best.b) {
            const b = best.b;
            S.tip = { x: X_(0) + 12, y: Y_(b.enc) - 34, html: `<div>${b.n} · dense, 0% zeros</div><div>encoder ${b.enc.toFixed(2)} · projector ${b.proj.toFixed(2)}</div>` };
          } else if (best) {
            const i = best.i;
            S.tip = { x: X_(zeros[i]) + 12, y: Y_(T13.enc[i]) - 34, html: `<div>${K(`\\mu = ${texNum(T13.mu[i])}`, 14)} · ${zeros[i].toFixed(1)}% zeros</div><div>encoder ${T13.enc[i].toFixed(2)} · projector ${T13.proj[i].toFixed(2)}</div>` };
          }
        }
      ...
      function drawTip() {
        const t = S.tip;
        if (!t) return;
        // flip left before the box would reach the tables (step 5), the panel (steps 2–3) or the viewport edge
        const w = measure('tip').w || 240;
        let limit = L.w - 16;
        if (!L.mob && S.step === 4 && L.pa && L.pa.tables) limit = L.pa.tx - 8;
        const right = t.x + w > limit;
        lab('tip', t.html, right ? t.x - 26 : t.x, Math.max(70, t.y), { ax: right ? 1 : 0, ay: 0, cls: 'tipbox' });
      }
