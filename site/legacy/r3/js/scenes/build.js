 ...
 * ([1] LpWM sparse latent rollout, [2] SkyWindFarm turbines in the sky, [3] Rectified LpJEPA histogram,
 * [4] Radial-VCReg burst), Paradigm-film hex dumps / node grids and project photos cut in on the beat.
 ...
 *   s.typing([sec])     on-screen keystroke times: the score drops its click row around them
...
(function () {
  ...
  const HOVER = {
    'builders': { mode: 'flip', frames: MONTAGE, title: 'Things I’ve built', meta: '2022 → 2026 · hover any blue phrase' },
...
.scene--build .bd-sh > span:first-child { color: #555; text-transform: uppercase; letter-spacing: .06em; } /* eyebrow: INDEX · 18 PROJECTS */
...
`;
  ...
  Site.register({
    ...
    create(el, api) {
      ...
      /* ---------------------------------------------------------- vignettes */
      // Scene [2] — SkyWindFarm: LTA units rise on tethers into high-altitude wind (illustrative).
      // unitArt() is the one place a unit is drawn: Site.art.swfUnit from skywindfarm.js (the same glyph as
      // scene [2]; anchor = deck centre, shell above, rotors below, returns the bridle point), with a local
      ...
      function vCode(R, lt, t, o) {
        ...
        const capL = sceneRef('', 1, ' LpWM · sparse beats dense by up to +57 pts on PushT planning'), long = !narrow && x0 + mixW(capL) <= G.F.x1 + 4;
        labelMix(long ? capL : sceneRef('', 1, ' LpWM · sparse up to +57 pts on PushT'), x0, y0 + gh + (narrow ? 40 : 30));
      }
      ...
      function vRadial(R, lt, t, o) {
        ...
        const capV = [nw ? ' Radial-VCReg · toy' : ' Radial-VCReg · 2-D toy · illustrative', ' Radial-VCReg · toy', ' Radial-VCReg']
          .map(q => [['fig. 07 · ', T2], ['[4]', C.accent], [q, T2]]).concat([[['[4]', C.accent], [' Radial-VCReg', T2]]]);
      ...
      function vHist(R, lt, t, o) {
        ...
        labelMix(sceneRef('', 3, nw ? ' Rectified LpJEPA · illustrative' : ' Rectified LpJEPA · rectified Gaussian target · illustrative'), R.x, R.y + 32);
      }
      ...
      // One obstacle of the flow figure: a SkyWindFarm unit (the scene [2] glyph) centred in its model cylinder
      ...
      SEC[0] = (lt, t) => {
        ...
        if (lt >= FLIP.t0) {
          ...
          const fy = r.bottom + (G.mobile ? 26 : 40);
          const fw = Math.round(G.mobile ? F.w * 0.84 : Math.min(F.w * 0.58, (F.y1 - fy - 30) / 0.66)), fh = Math.round(fw * 0.66);
          const fx = G.mobile ? TX : Math.round(F.x1 - fw);
          const NM = MONTAGE.length, figCap = 'fig. 00 — things I’ve built';
        ...
        if (lt < SP.s0.at) cursor(TX, y, size, lt < BEAT * 2, t); // solid for the first two beats: the first paint is never empty
      };
      ...
      SEC[1] = (lt, t) => {
        ...
        if (lt < lb(3)) {
          ...
          const capV = [' Rectified LpJEPA · same data, 4 projections', ' LpJEPA · same data, 4 projections', ' LpJEPA · 4 projections', ' LpJEPA'].map(q => sceneRef('', 3, q));
      ...
      SEC[2] = (lt, t) => {
        ...
        if (lt < lb(3)) {
          ...
          labelMix(sceneRef('“question everything” → ', 2, G.mobile ? ' SkyWindFarm' : ' SkyWindFarm, 2022–2024'), TX, F.y0 + 14);
      ...
      SEC[4] = (lt, t) => {
        ...
        if (lt < lb(3)) {
          ...
          if (lt < lb(2)) {
            ...
            const fpV = [' SkyWindFarm · from first principles', ' SkyWindFarm · first principles', ' SkyWindFarm'].map(q => sceneRef('', 2, q));
          ...
          const fpV = [['“first principles” · “non-mimetic” → ', ' LpWM, a sparse world model'], ['“first principles” · “non-mimetic” → ', ' LpWM'],
            ['“first principles” → ', ' LpWM']].map(([p, q]) => sceneRef(p, 1, q));
      ...
      function vEnergy(R, u, t, o) {
        ...
        U.forEach((q, i) => {
          ...
          // the unit (scene [2] glyph): its generator plates light as power starts to flow
        ...
        if (hd > 0) { // the scene reference, kept inside the frame (a shorter wording where the long one would run past it)
          ...
          const long = sceneRef('', 2, ' SkyWindFarm · power flows down the tether'), short = sceneRef('', 2, ' SkyWindFarm · power down the tether');
          if (mob) { // (stacked: the wire from the station up to the phrase runs at x = o.wireX — the label sits clear of it)
            const tiny = sceneRef('', 2, ' SkyWindFarm'), clear = (q, x0) => o.wireX == null || o.wireX < x0 - 10 || o.wireX > x0 + mixW(q) + 10;
      ...
      SEC[10] = (lt, t) => {
        const { TX, TY, TW, sz, FIG, F } = G;
        if (lt < lb(3)) {
          const size = sz.L, tw10 = G.mobile ? TW : Math.max(TW, F.w * 0.8); // (nothing beside these lines: the strip is under them)
          // The strip of four panels starts under the two lines' final layout: lower where they need it (down to
          // its minimum height), and where even that is not enough the two lines start higher.
          ...
          // bar 42: the strip recedes (fades, drifting a few px left — it stays inside the frame's clip)
          const drift = lt >= lb(2) ? eIn((lt - lb(2)) / BAR) : 0;
          ctx.save(); ctx.globalAlpha = 1 - drift * 0.88; ctx.translate(-drift * 8, 0);
          ...
          const cells = [
            [1, 'LpWM', (r) => vCode(r, BEAT * 7.5, t, { mini: true, fill: true })],
            [2, 'SkyWindFarm', (r) => vTurb(r, BAR * 2, t, { mini: true })],
            [3, 'Rectified LpJEPA', (r) => vHist(r, BAR * 1.5 + lt, t, { mini: true, fill: BAR, rect: BAR })],
            [4, 'Radial-VCReg', (r) => vRadial(r, BAR + 0.1 + lt * 0.8, t, { mini: true, burst: BAR })],
          ];
          ...
          afterFade = () => {
            const cw = G.mobile ? F.w : Math.min(F.w, 900), fs = 12, lhf = 1.4;
            ...
            if (lt >= SP.s10f.at) { const k = eOut((lt - SP.s10f.at) / BEAT); seg(F.x0, yf - 22.5, F.x0 + 48 * k, yf - 22.5, C.g400); }
            typed(SP.s10f, lt, F.x0, yf, fs, cw, { mono: true, color: T2, lhf });
      ...
      function getScore() {
        ...
        // the score steps its euclidean click row aside around every on-screen keystroke and slam
        if (score && typeof score.typing === 'function') { try { score.typing(FOLEY.filter(e => e.k === 'key' || e.k === 'slam').map(e => e.t)); } catch (e) {} }
        return score;
      }
      ...
      function foleyFire(e, when) {
        ...
        switch (e.k) {
          ...
          case 'key': if (!play('click', { when, gain: 0.6 * (0.9 + 0.2 * ((e.t * 7919) % 1)), freq: 1700 + ((e.t * 3971) % 1) * 3300, pan: pan * 0.4, dest })) play('hat', { when, gain: 0.05, dest }); break;
          case 'slam': if (!play('glitch', { when, repeats: 3, len: 0.018, gain: 0.36, dest })) play('click', { when, gain: 0.3, freq: 1800, dest }); break;
      ...
      function settle(why) {
        ...
        else { clearTimeout(tailTimer); tailTimer = setTimeout(() => { if (mode === 'settled' && sPlaying()) scoreStop(2.5); }, 6000); }
        ...
        bedTimer = setTimeout(() => { if (mode === 'settled' && soundOn && !bed && !bedOff) startBed(); }, why === 'end' ? 2600 : 900); // (sound(true) may have started it already)
        if (why === 'skip') ui('open');
      }
      ...
      // Timeline: projects per year. Each project is one small square in its start year's stack (two columns, bottom
      ...
      const hintT = S('text', { class: 'hn', x: '0', y: '11' }); hintT.textContent = TOUCH ? 'projects per year' : 'projects per year · hover a row'; tl.append(hintT);
      ...
      const SCENE_OF = { lpwm: ['lpwm', 1], skywindfarm: ['skywindfarm', 2], 'rectified-lpjepa': ['lpjepa', 3], 'radial-vcreg': ['radial-vcreg', 4] };
      ...
      // the live 'will power' card: three units (the scene [2] glyph) aloft in the high wind, their generator plates
