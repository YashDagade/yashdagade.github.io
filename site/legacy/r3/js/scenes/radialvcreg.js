/* yashdagade.com v2 — scene [4] Radial-VCReg.
 *
 ...
 *
 * Round 2: all notation is KaTeX (head prose, key formula per step, readouts and in-canvas labels via
 * an HTML overlay layer, slider label/ends); text ≥ 12px in #000/#555. Sound: grains, bits, subs and
 * every sustained voice stay in D3–A4; only 2–8 ms ticks (never sustained) reach D4–A5.
...
(function () {
  ...
  // Step copy: `text` / `title` are "rich" strings — $…$ segments are typeset with KaTeX, the rest is prose.
  // `eq` is the step's key formula (KaTeX display math), `tag` names where it comes from.
  // One name per step: the stepper label and the head title are the same words.
  // Step 1's key formula is live: the definition of Σ̂ followed by its current value (`live`).
  // `textM` is the phone variant of the prose (tap instead of click).
  const STEPS = [
    { label: 'Raw features', title: 'Raw features', dur: 7.5, settle: 3.5,
      text: 'An encoder’s features: correlated, stretched and not Gaussian. Click the cloud to change its shape; drag across it to reshape it.',
      textM: 'An encoder’s features: correlated, stretched and not Gaussian. Tap the cloud to change its shape; drag across it to reshape it.',
      eq: '\\hat\\Sigma = \\frac{1}{N-1}\\sum_i\\,(z_i-\\bar z)(z_i-\\bar z)^{\\top}', live: true, tag: '' },
    { label: 'Whiten (VCReg)', title: 'Whiten (VCReg)', dur: 12.5, settle: 11,
      text: 'VCReg whitens, so $\\hat\\Sigma \\to I$ and its loss falls to 0. But second-order statistics cannot see shape: an X and a Gaussian share $\\Sigma = I$.',
      eq: 'z \\;\\mapsto\\; \\hat\\Sigma^{-1/2}\\,(z-\\hat\\mu)', tag: 'whitening' },
    { label: 'Chi shell', title: 'Chi shell', dur: 10.5, settle: 7.5,
      text: 'The norm of a $d$-dim Gaussian follows $\\chi_d$: a thin shell at radius $\\approx\\!\\sqrt{d}$, width $\\approx 0.71$. Checking it is a 1-D problem, whatever $d$ is.',
    ...
    { label: 'Radial map', title: 'Radial map', dur: 14, settle: 4.6, settleRM: 10.5,
      text: 'Keep each direction; remap the radius so $\\|z\\| \\sim \\chi_2$. Elliptical shapes become $\\mathcal N(0, I)$. Others get the radius right and the angles wrong.',
      eq: 'T(z) = \\frac{z}{\\|z\\|}\\;F_{\\chi_2}^{-1}\\!\\big(F_{\\|z\\|}(\\|z\\|)\\big)', tag: 'Prop. 1 · $z$ whitened' },
    { label: 'Toy experiment', title: 'Toy experiment', dur: 12, settle: 7.5,
      text: 'X-samples optimized for 200k SGD steps with the Radial-VCReg loss (paper Fig. 1). VCReg alone barely moves them.',
      eq: '\\mathcal L_{\\mathrm{VCReg}} + D_{\\mathrm{KL}}\\big(p(\\|z\\|)\\,\\big\\|\\,\\chi_2\\big)', tag: 'Eqs. 2, 8' },
    { label: 'Real features', title: 'Real features', dur: 14, settle: 9,
      text: '$\\|z\\|$ of 512-dim image features against $\\chi_{512}$. The radial term moves the norms onto the shell; probe gains are largest at $d = 512$.',
...
.scene--radial-vcreg .rv-kick { display: flex; align-items: center; gap: 10px; }
...
/* shared step counter (all scenes): "01 / 06 · Step name", a short progress hairline, the play state.
   Desktop: top-right at the 40 px gutter. Phones: the first line of the step head, beside ‹ ›. */
...
.scene--radial-vcreg .rv-kick .rv-step { position: static; text-align: left; }
.scene--radial-vcreg .rv-kick .rv-step .rv-prog { margin: 4px 0 0; }
...
.scene--radial-vcreg .rv-title { font-family: var(--serif); font-weight: 400; font-size: 24px; line-height: 28px; letter-spacing: -.015em; margin: 0 0 7px; color: var(--ink); }
/* cramped desktop columns: the phone head type, then no key formula (see fitRight) */
.scene--radial-vcreg .rv-head.is-compact .rv-title { font-size: 20px; line-height: 24px; margin-bottom: 4px; }
.scene--radial-vcreg .rv-head.is-compact .rv-text { font-size: 15px; line-height: 19px; }
.scene--radial-vcreg .rv-head.is-compact .rv-eq { margin-top: 8px; font-size: 14px; }
.scene--radial-vcreg .rv-head.is-noeq .rv-eq { display: none; }
.scene--radial-vcreg .rv-title .katex { font-size: .98em; }
.scene--radial-vcreg .rv-text { margin: 0; font-family: var(--serif); font-size: 16px; line-height: 21px; color: #333; }
.scene--radial-vcreg .rv-text .katex { font-size: 1.04em; line-height: 1; }
.scene--radial-vcreg .rv-eq { display: flex; flex-wrap: wrap; align-items: center; gap: 4px 16px; margin-top: 14px; font-size: 16px; line-height: 1.2; color: var(--ink); }
...
.scene--radial-vcreg .rv-title, .scene--radial-vcreg .rv-text, .scene--radial-vcreg .rv-eq { transition: opacity .28s ease, transform .28s cubic-bezier(.2,.7,.2,1); }
.scene--radial-vcreg .rv-head.is-swap .rv-title, .scene--radial-vcreg .rv-head.is-swap .rv-text, .scene--radial-vcreg .rv-head.is-swap .rv-eq { opacity: 0; transform: translateY(3px); }
.scene--radial-vcreg .rv-nav { display: none; pointer-events: auto; margin-left: auto; gap: 6px; }
.scene--radial-vcreg .rv-nav button { position: relative; font-size: 13px; line-height: 16px; padding: 5px 11px; background: var(--g200); border-radius: 13px; color: var(--ink); }
.scene--radial-vcreg .rv-nav button::after { content: ''; position: absolute; inset: -9px -3px; }
...
@media (prefers-reduced-motion: reduce) {
  .scene--radial-vcreg .rv-title, .scene--radial-vcreg .rv-text, .scene--radial-vcreg .rv-eq, .scene--radial-vcreg .rv-opts { transition: none; }
}
@media (max-width: 800px) {
  .scene--radial-vcreg .rv-nav { display: flex; }
  .scene--radial-vcreg .rv-title { font-size: 20px; line-height: 24px; margin: 6px 0 4px; }
  .scene--radial-vcreg .rv-text { font-size: 15px; line-height: 19px; }
  .scene--radial-vcreg .rv-eq { margin-top: 8px; font-size: 14px; }
  .scene--radial-vcreg .rv-opts { gap: 12px; }
  .scene--radial-vcreg .rv-head.is-slim .rv-kick { position: absolute; right: 0; top: -1px; }
  .scene--radial-vcreg .rv-head.is-slim .rv-title { margin-top: 0; }
...
`;
  ...
  Site.register({
    ...
    title: 'Radial-VCReg',
    path: '/radial-vcreg',
    caption: 'Whitening only fixes 2nd-order stats.\nRadial-VCReg also matches feature\nnorms to the chi law of a Gaussian.',
    ...
    create(el, api) {
      ...
      const kick = h('div', 'rv-kick');   // phones: the counter row of the step head, with ‹ ›
      ...
      kick.append(nav);
      const counterText = () => `0${step + 1} / 0${STEPS.length}` + (L.mobile ? '' : ` · ${STEPS[step].label}`);
      function placeCounter() {
        if (L.mobile) { stepBox.append(kNum, kState, prog); if (stepBox.parentNode !== kick) kick.insertBefore(stepBox, nav); }
      ...
      function fillHead(s, mat) {
        hTitle.innerHTML = rich(s.title);
        hText.innerHTML = rich(L.mobile && s.textM ? s.textM : s.text);
        setEq(eqOf(s, mat));
      ...
      // χ(d) portrait cache: r (true χ_d radius) per point, coupled by the same uᵢ across d
      const chiCache = new Map();
      function chiRadii(d) {
        let c = chiCache.get(d);
        if (!c) { c = new Float64Array(N); for (let i = 0; i < N; i++) c[i] = chiQuant(UU[i], d); chiCache.set(d, c); }
        return c;
      }
      ...
      // Pointer
      const P = { x: -1, y: -1, over: null, down: false, id: null, sx: 0, sy: 0, lx: 0, ly: 0, moved: false, touch: false, near: -1, bin: -1, col: -1, row: -1, stick: -9 };
      ...
      // Head modes. Desktop: 'full'; cramped columns fall back to 'compact' (the phone head type), then
      // 'noeq' / 'cnoeq' (no key formula). Phones: 'mobile' (counter row above the title), 'slim' (short
      // phones: the counter shares the title's row).
      function headClasses(node, mode) {
        const desk = mode !== 'mobile' && mode !== 'slim';
        node.classList.toggle('is-desk', desk);
        node.classList.toggle('is-compact', mode === 'compact' || mode === 'cnoeq');
        node.classList.toggle('is-noeq', !desk || mode === 'noeq' || mode === 'cnoeq');
        node.classList.toggle('is-slim', mode === 'slim');
      }
      // Tallest head (title + prose + key formula, plus the counter row on phones) over all steps at the
      // current column width and mode, so the right column never jumps between steps.
      function measureHeads(width, mode) {
        ...
        const pT = probe.querySelector('.rv-title'), pX = probe.querySelector('.rv-text'), pE = probe.querySelector('.rv-eq'), pM = probe.querySelector('.rv-eqm'), pG = probe.querySelector('.rv-eqtag');
        ...
        for (const s of STEPS) {
          pT.innerHTML = rich(s.title); pX.innerHTML = rich(L.mobile && s.textM ? s.textM : s.text);
          if (withEq) { api.tex(pM, eqOf(s), { display: true }); pG.innerHTML = rich(s.tag); pG.style.display = s.tag ? '' : 'none'; }
          mx = Math.max(mx, probe.offsetHeight);
        }
        L.eqShort = keep;
        probe.remove();
        return { h: mx || 190, short };
      }
      ...
      const relayout = () => { headMemo.clear(); ctrWMemo = 0; layout(); }; // fonts arrived: measure again

      // Desktop, landscape: controls | cloud | right column (head, readouts, histogram panel).
      function layoutSide(w, hh) {
        const CR = L.CR;
        // narrow windows: a smaller cloud keeps the right column ≥ 280 px wide
        const u = Math.max(26, Math.min(0.069 * hh, 0.043 * w, (w - 341 - CR) / 8.95, (0.57 * w - 320) / 4.5));
        L.u = u;
        ...
        // the head column starts under the step counter when the two share x
        const topMin = L.headX + L.headW > w - 40 - ctrWidth() - 16 ? CTR_BOTTOM + 8 : 76;
        // head + 20 px + the histogram title must fit above the plot: first by lowering the panel a little
        // and shortening it (≤ 20 %), then with the phone head type, then without the key formula
        // (the panel keeps its baseline on the frame's bottom while the phone head type is enough)
        let pick = null;
        for (const [mode, move] of [['full', 0], ['compact', 0], ['full', 1], ['compact', 1], ['noeq', 1], ['cnoeq', 1]]) {
          const m = headHeight(L.headW, mode);
          const room = (oy, dp) => oy - 0.66 * dp - 53 - topMin - m.h;
          let oy = oy0, dp = dp0;
      ...
      // Desktop, portrait (iPad): the head across the top, then the cloud, then the histogram panel.
      function layoutPortrait(w, hh) {
        const CR = L.CR;
        L.headX = CR + 10; L.headW = Math.min(560, w - 40 - L.headX);
        const topMin = L.headX + L.headW > w - 40 - ctrWidth() - 16 ? CTR_BOTTOM + 8 : 76;
        ...
        let pick = null;
        for (const mode of ['full', 'compact', 'noeq', 'cnoeq']) {
          const m = headHeight(L.headW, mode);
          const avail = hh - 140 - (topMin + m.h + 18) - 174 - 0.66 * dp;
        ...
        if (R) slack -= R + 14;
        L.infoX = L.headX; L.infoW = L.headW;
        L.infoB = R ? topMin + L.headH + 20 + R : -1;   // no block: no readout rows at all
        L.pt = 1.5; L.row = 18;
        const labelTop = topMin + L.headH + 18 + (R ? R + 14 : 0) + slack / 2;
      ...
      // Phones: one column, laid out bottom-up from the slider the shell places above the caption.
      function layoutMobile(w, hh) {
        ...
        L.headX = 16; L.headY = 74; L.headW = w - 32;
        L.dp = clamp(0.1 * hh, 70, 96);
        L.oy = sliderTop - 34;
        L.ox = 16; L.pw = w - 34;
        L.ptY = L.oy - 0.66 * L.dp - 16;
        L.row = 17; L.pt = 1.25;
        L.infoX = 16; L.infoW = w - 32;
        // box bottom: opts row + honesty tag + one live readout row above the panel title; on short
        // phones ('slim') the counter shares the title's row and the tag and the readout row go
        const boxB = slim => (slim ? L.ptY - 44 : L.ptY - 17 - L.row - 60);
        let pick = { mode: 'mobile', m: headHeight(L.headW, 'mobile') };
        if ((boxB(false) - (L.headY + pick.m.h + 14)) / 7 < 30) pick = { mode: 'slim', m: headHeight(L.headW, 'slim') };
        applyHeadMode(pick);
        L.slim = pick.mode === 'slim';
        L.infoB = L.ptY - 17;
        L.infoY = L.slim ? L.infoB : L.infoB - L.row;
        const boxBottom = boxB(L.slim), top = L.headY + L.headH + 14;
        L.u = Math.max(L.slim ? 19 : 22, Math.min((w - 40) / 7.2, (boxBottom - top) / 7));
        L.cx = w / 2;
      ...
      function applyHeadMode(pick) {
        ...
        L.headMode = pick.mode; L.headH = pick.m.h;
      ...
      function layout() {
        ...
        if (L.mobile) layoutMobile(w, hh); else if (L.portrait) layoutPortrait(w, hh); else layoutSide(w, hh);
        L.box = 3.5 * L.u;
        L.margH = 0.95 * L.u;
        ...
        tagWrap();
        if (!L.mobile) {
          // readouts flow right under the current step's head (see the ResizeObserver below)
          const hc = head.offsetHeight;
          L.headCur = hc > 40 ? hc : L.headH;
          L.infoY = L.headY + L.headCur + 20;
        }
        opts.style.left = L.cx + 'px'; opts.style.top = L.optsY + 'px';
      ...
      function portraitView(dt) {
        ...
        if (L.portrait && !L.mobile) {
          const hC = ccdH(ccdGeom(L.infoW), 4, 19, true), top = L.headY + (L.headCur || L.headH) + 20;
      ...
      let soundOn = false, clockOff = null, hinted = false;
      const lastT = {};
      ...
      function update(dt) {
        ...
        if (!hinted && clockT - enterT > 1.5) {
          ...
          if (a && a.ready) {
            ...
            else if (clockT - readyAt > 0.5) { hinted = true; api.hint(L.mobile ? 'Tap the cloud to change its shape · drag to reshape' : 'Click the cloud to change its shape · drag to reshape · arrow keys step · space pauses', 4600); }
          }
        }
        ...
        progBar.style.transform = `scaleX(${clamp(stepT / st.dur, 0, 1).toFixed(4)})`;
        ...
        // (a viewer's input holds the step, or reduced motion), 'paused · [space]'. Phones never show the key;
        // short phones (counter on the title's row) name only a paused timeline, the gray hairline says the rest
        const cue = api.reduced ? 'r' : paused ? 'p' : hold ? 'h' : 'a';
        const cueKey = cue + (L.mobile ? 'm' : '') + (L.slim ? 's' : '');
        if (cueKey !== cueShown) {
          cueShown = cueKey;
          kState.textContent = L.mobile
            ? (L.slim ? ({ p: 'paused' })[cue] || '' : ({ r: 'interactive', p: 'paused', h: 'interactive', a: 'autoplay' })[cue])
            : ({ r: 'interactive', p: 'paused · [space]', h: 'interactive', a: 'autoplay' })[cue];
          prog.classList.toggle('is-still', cue !== 'a');
        }
        const hovering = figHover || P.bin >= 0 || P.row >= 0 || P.col >= 0;
        if (auto && !paused && !hold && !hovering && !api.reduced && stepT >= st.dur) {
      ...
      function drawCloudFrame() {
        ...
        // marginal lives above the frame)
        if (x1 + 24 < L.w - 4) { T('ax1', '$z_1$', x1 + 7, L.cy, { c: SEC }); plRect(x1 + 5, L.cy - 9, 22, 18); }
        else { T('ax1i', '$z_1$', x1 - 6, L.cy - 13, { a: 'r', c: SEC, cls: 'tag' }); plRect(x1 - 28, L.cy - 22, 24, 18); }
      ...
      function drawMarginals(alpha) {
        ...
        const clearOfControls = yb - 8 > 0.7 * L.h + 12 || xr - kw > L.CR + 8;
        if (xr >= left - mh && xr - kw >= 16 && clearOfControls) text(ks, xr, yb, SEC, 'right');
      ...
      function drawTag() {
        ...
        // desktop: left-aligned under the frame, the longest variant that stays clear of the histogram panel
        const vs = step === 2 ? ['portrait of $\\mathcal N(0, I_d)$: random angle, radius $\\|z\\|\\sqrt{2/d}$', 'portrait: random angle, radius $\\|z\\|\\sqrt{2/d}$', 'portrait: radius $\\|z\\|\\sqrt{2/d}$']
          : step === 4 ? ['end state: paper Fig. 1b · morph path schematic', 'end state Fig. 1b · path schematic', 'end state Fig. 1b']
          : step === 5 ? (chartCInCloud() ? ['bars: paper Fig. 2a–c, Tables 1–3', 'bars: Tables 1–3'] : ['bars: paper Fig. 2a–c · cloud illustrative', 'bars: Fig. 2a–c · cloud illustrative', 'bars: Fig. 2a–c'])
          : ['illustrative · 1,500 samples · live numbers', 'illustrative · 1,500 samples · live', 'illustrative · live'];
        noteAt('tag', vs, x, L.tagY, SEC, L.tagMax);
      }
      ...
      // closer to the head on this step) when that holds its four benchmark rows; else (≈ 1100 px windows and
      // below) it takes the cloud's place, as on phones (that portrait is illustrative). Portrait windows
      // (iPad) make the room under the head by shrinking the cloud frame on this step (portraitView).
      const infoTop = () => L.infoY - (step === 5 && !L.mobile && !L.portrait && L.infoB - L.infoY < ccdNeed(ccdGeom(L.infoW)) ? 8 : 0);
      const chartCInCloud = () => L.mobile || (L.portrait ? !L.cc6 : L.infoB - infoTop() < ccdNeed(ccdGeom(L.infoW)));
      function drawRPanel() {
        hoverLine = '';
      ...
      function noteAt(key, variants, x, y, c, maxX, cls) {
        // (never past the chrome's gutter: 40 px on desktop, 16 px on phones)
        const vs = Array.isArray(variants) ? variants : [variants], right = Math.min(maxX || Infinity, L.w - (L.mobile ? 16 : 40));
        for (let j = 0; j < vs.length; j++) {
          const s = vs[j], k = j ? key + '_' + j : key, isTex = !!cls || s.indexOf('$') >= 0;
          let w;
          if (isTex) { T(k, s, -9999, -9999, { c: c || SEC, cls }); w = tw(k); } else w = textW(s);
          if (x + w <= right) {
            if (isTex) T(k, s, x, y, { c: c || SEC, cls }); else text(s, x, y, c || SEC);
            return true;
          }
        }
        return false;
      ...
      function drawReadouts() {
        ...
        // target and the one payoff label of each step. Notes: each drawn only if it fits (else a shorter one).
        const row = (key, label, value, reached, note) => {
          ...
          if (note) noteAt('rn' + key, note, nx, y);
          yTop += rh;
        };
        ...
        if (step === 2) {
          ...
          fit([
            ...
            { h: rh, p: 5, d: () => row('mo', 'mode $\\sqrt{d-1}$', fmt(ci.mode, 2), false, 'where $\\|z\\|$ peaks') },
            { h: rh, p: 4, d: () => row('ms', 'mean · sd', `${fmt(ci.mean, 2)} · ${fmt(ci.sd, 2)}`, false, 'width stays $O(1)$') },
            { h: rh, p: 2, d: () => row('rel', 'sd / mean', fmt(ci.sd / ci.mean, 3), false, 'relative width $\\to 0$') },
          ]);
          return;
        }
        if (step === 5) {
        ...
        if (step !== 0) rows.push({ h: 46, p: 9, d: () => {
          const y = yTop + 20;
          ...
          noteAt('rnsig', whiteOK ? 'identity: whitened' : nearI ? (mid ? 'near identity' : 'near identity (noise)')
        ...
        if (step <= 1) rows.push({ h: rh, p: step === 0 ? 8 : 3, d: () => row('sg', '$\\sigma_1\\,\\cdot\\,\\sigma_2$', `${fmt(ST.s1, 2)} · ${fmt(ST.s2, 2)}`, sgOK, 'principal axes of $\\hat\\Sigma$') });
        rows.push(
          { h: rh, p: pr.vc, d: () => row('vc', '$v(Z)\\,\\cdot\\,c(Z)$', `${fmt(ST.v, 3)} · ${fmt(ST.cz, 3)}`, vcOK, 'VCReg terms, Eq. 1') },
          { h: rh, p: pr.ku, d: () => row('ku', 'kurtosis', `${fmt(ST.kx, 2)} · ${fmt(ST.ky, 2)}`, kOK, 'a Gaussian has 3') },
        );
        if (step !== 0) rows.push(
          { h: rh, p: pr.w1, d: () => row('w1', '$W_1(\\|z\\|, \\chi_2)$', w1, rOK, 'radius vs. chi law') },
          // the m-spacing entropy estimate is biased at finite N: an exact χ₂ sample still scores ≈ 0.05
          { h: rh, p: pr.rl, d: () => row('rl', 'radial loss $r(Z)$', ls, false, [`Eq. 7 · $\\approx 0$ up to estimator bias`, `Eq. 7, $m = ${M_SP}$`, 'Eq. 7']) },
        );
        fit(rows);
      }
      ...
      function draw() {
        ...
        if (step === 5 && chartCInCloud()) {
          ...
          if (L.mobile) drawChartC(16, L.cy - L.box + 2, L.cy + L.box, L.w - 32);
            ...
            // a small frame (narrow windows) lends the chart the left marginal's strip (hidden in this
            // step), never the control column
            let cx0 = L.cx - L.box + 4;
            if (2 * L.box - 4 < 300) cx0 = Math.min(cx0, Math.max(L.CR + 12, L.cx - L.box - L.margH - 9));
            // (right: clear of the histogram's density numbers when the panel sits beside the frame)
            const cx1 = L.portrait ? L.cx + L.box : Math.min(L.cx + L.box, L.ox - 44);
            drawChartCD(cx0, L.cy - 0.62 * L.box, L.cy + L.box, cx1 - cx0);
          }
        } else {
      ...
      function region(x, y) {
        ...
        if (Math.abs(x - L.cx) <= L.box + 4 && Math.abs(y - L.cy) <= L.box + 4) return 'cloud';
      ...
      // a drag on the cloud must not be read by the shell as a scene swipe
      ...
      cnv.addEventListener('touchend', e => { if (P.moved || region(e.changedTouches[0].clientX, e.changedTouches[0].clientY) === 'cloud') e.stopPropagation(); }, { passive: true });
      ...
      layout();
      api.onResize(() => layout());
      // KaTeX / webfont metrics change the head's height once they load: re-measure then
      if (document.fonts && document.fonts.ready) document.fonts.ready.then(() => { relayout(); });
      if (window.ResizeObserver) {
        new ResizeObserver(() => {
          tagWrap();
          const hc = head.offsetHeight;
          if (!L.mobile && hc > 40) { L.headCur = hc; L.infoY = L.headY + hc + 20; }
        }).observe(head);
      }
      ...
      el.__rv = { ST, L, get step() { return step; }, get stepT() { return stepT; }, get hold() { return hold; }, P, seek(i, t) { goStep(i, true); stepT = t || 0; } };

      return {
        enter() {
          shown.v6 = 0;
          layout();
          requestAnimationFrame(() => { if (api.isActive()) { layout(); decorateSlider(); } });
          // KaTeX webfonts arrive lazily with the first formula: settle the column once they have
          setTimeout(() => { if (api.isActive()) relayout(); }, 1400);
        ...
        exit() {
          P.down = false; P.over = null; P.near = -1; P.bin = -1; P.col = -1; P.row = -1;
          clearTimeout(swapTimer);
        },
        sound(on) {
          const a = AU();
