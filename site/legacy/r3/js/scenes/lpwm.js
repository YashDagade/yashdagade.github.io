...
(function () {
  ...
  const CAPS = [
    'A world model: an encoder, a predictor\nand a planner that searches actions\nin imagination, then acts (MPC).',
    'The encoder is a ViT: patches become\ntokens, 12 attention blocks mix them,\nthen CLS → MLP → ReLU = sparse z.',
    'A ReLU link and a Rectified Laplace\ntarget leave about half of z exactly\nzero. Dense LeWM: every unit active.',
    'The predictor rolls the sparse code\nforward under actions. Rollout error\ncompounds with horizon (Lemma G.1).',
    'Plan in imagination: CEM scores 300\naction sequences, executes 5 steps,\nthen replans. Drag the goal or agent.',
    'Same small predictor, sparse vs dense:\nMLP∘LTI(k) plans 62.7% vs 5.3% on\nPushT (open-loop, D = 4096).',
    'Planning further ahead is harder for\nboth, but sparse LpWM stays ahead\nat every horizon (Piecewise 2×2).',
  ];
  ...
  Site.register({
    ...
    create(el, api) {
      ...
      /* ---------------------------------------------------------------- layout */
      // Four layouts, each drawn at a scale near 1 (labels are fixed-size px, so the art must not shrink far
      // below its design size or the words collide):
      //   wide  1060×600  roomy desktops (≥ ~1230 × 700)
      //   mid    620×420  small landscape windows (1100×700, 1024×640, 900×620): fewer, shorter labels
      //   tall   358×560  the phone composition on a portrait tablet (834×1112), with the desktop chrome
      //   phone  358×560  phones (≤ 800 px, core's mobile chrome); short phones (375×667) drop secondary rows
      // G.mobile = the stacked (phone) composition, G.phone = the phone chrome, G.mid, G.short.
      let G = null;
      function relayout() {
        const w = cv.w, h = cv.h;
        let lay, dw, dh, ax0, ax1, ay0, ay1, smax = 1.22;
        const col = w < 1100 ? 260 : 300; // clear of the left control column (stepper + slider)
        if (w <= 800) { lay = 'phone'; dw = 358; dh = 560; ax0 = 16; ax1 = w - 16; ay0 = 104; ay1 = h - 176; }
        else if (Math.min((w - 356) / 1060, (h - 196) / 600) >= 0.82) { lay = 'wide'; dw = 1060; dh = 600; ax0 = 300; ax1 = w - 56; ay0 = 104; ay1 = h - 92; }
        else if (h > w * 1.1) { lay = 'tall'; dw = 358; dh = 560; ax0 = col; ax1 = w - 40; ay0 = 150; ay1 = h - 100; smax = 1.35; }
        // (mid starts under the step header's play-state line, baseline 110: the Plan step's panel titles sit
        // right under it on narrow windows)
        else { lay = 'mid'; dw = 620; dh = 420; ax0 = col; ax1 = w - 40; ay0 = 124; ay1 = h - 96; smax = 1.15; }
        let s = clamp(Math.min((ax1 - ax0) / dw, (ay1 - ay0) / dh), 0.5, smax);
        // short phones: fit the width and let each step drop its secondary rows instead of shrinking
        const short = lay === 'phone' && (ay1 - ay0) / dh < 0.94;
        // (the short compositions need ≈ 387 units above the slider; very short windows scale them a little)
        if (short) s = clamp(Math.min((ax1 - ax0) / dw, (h - 294) / 387), 0.5, 1);
        G = {
          w, h, lay, s, short, mobile: lay === 'phone' || lay === 'tall', phone: lay === 'phone', mid: lay === 'mid',
          ox: ax0 + (ax1 - ax0 - dw * s) / 2, oy: ay0 + (ay1 - ay0 - dh * s) / 2,
        };
        // (phones sit high, under the step header; the tall iPad-portrait composition is centred in its band)
        if (G.mobile) G.oy = ay0 + Math.max(0, (ay1 - ay0 - dh * s) * (lay === 'tall' ? 0.5 : 0.3));
        if (short) G.oy = ay0;
        G.oy0 = G.oy;
        // every desktop composition starts on the content-left line (just right of the control column), under
        // the thesis, rather than floating centred in the leftover width
        if (lay !== 'phone') G.ox = ax0;
      ...
      const HOOK = 'Make the code sparse, and the world becomes simpler to predict.'; // (brief §1)
      const HOOK_LINES = ['Make the code sparse,', 'and the world becomes simpler to predict.'];
      let hookShown = 0;
      ...
      function view0(a, ta) {
        ...
        ctx.save();
        // the scene's thesis, typed on in the headline slot every scene shares (the content-left x of the art under
        // it: 300 px from 1100 px wide, 260 px below; top ≈ 90; phones have no room). It breaks after the comma
        // rather than run into the step header on the right.
        if (!G.phone) {
          const hp = reduced ? 1 : prog(ta, 0.15, HOOK.length / 36);
          const hx = cv.w >= 1100 ? 300 : 260;
          const lines = hx + textW(HOOK, F13) > hdrLeft() - 24 ? HOOK_LINES : [HOOK];
          let used = 0;
          lines.forEach((ln1, i) => {
            const pl = (hp * HOOK.length - used) / ln1.length;
            used += ln1.length;
            T(ln1, hx, 100 + i * 18, { font: F13, p: reduced ? 1 : pl });
          });
          T('LpWM · A Case for Sparse Representations in World Models', hx, 119 + (lines.length - 1) * 18, { color: SEC, p: prog(ta, 0.4 + HOOK.length / 36, 1.1), cursor: false });
          const n = Math.floor(HOOK.length * hp);
          if (S.step === 0 && n > hookShown) {
            if (Math.floor(n / 4) > Math.floor(hookShown / 4) && HOOK[n - 1] !== ' ' && audioLive() && A.ui && typeof A.ui.type === 'function') { try { A.ui.type({ gain: 0.28 }); } catch (e) { /* */ } }
            hookShown = n;
          }
        }
        // loop connectors
      ...
      function view3(a, ta) {
        ...
        if (G.mid) {
          ...
          TX('v3.hist', '$z_{t-2:t}$ · $k = 3$', X(176), Y(196), { size: 13, a });
          ...
          TX('v3.fb', 'fed back · $\\times H$ (autoregressive)', X(364), Y(14) - 11, { size: 12, color: rolling ? ACC : SEC, align: 'center', a, p: prog(ta, 1.0, 0.8) });
          ...
          if (plR) {
            ...
            T(R3.pct + '% active', R3.rxs[0], Y(380), { color: ACC });
            T('drift ' + R3.drift.toFixed(3), R3.rxs[0], Y(380) + 18, { color: SEC });
            T('100% active', R3.rxs[1], Y(380));
            T('drift ' + R3.otherDrift.toFixed(3), R3.rxs[1], Y(380) + 18, { color: SEC });
          }
      ...
      function view4(a, ta) {
        ...
        for (let k = 0; k < 2; k++) {
          ...
          const score = !done ? (m ? '—' : wide ? 'reached —' : '— reached') : m ? ag.ok + '/' + done : wide ? 'reached ' + ag.ok + ' of ' + done : ag.ok + '/' + done + ' reached';
        ...
        const ly = m ? Math.max(Y(G.short ? 286 : 376), capBottom + 22) : md ? Y(380) : Y(578);
      ...
      function view5(a, ta) {
        ...
        for (let i = 0; i < n; i++) {
          ...
          if (!m) {
            ...
            T(Math.round(LADDER.active[i] * 100) + '% active', xc(i), y1 + 55, { align: 'center', font: F12, color: SEC, a: la });
          } else {
            MN[i].split('|').forEach((s, k) => T(s, xc(i), y1 + 18 + k * 14, { align: 'center', font: F12, color: sel ? ACC : INK, a: la }));
            T(LADDER.params[i] + 'M', xc(i), y1 + 48, { align: 'center', font: F12, color: SEC, a: la });
            T(Math.round(LADDER.active[i] * 100) + '%', xc(i), y1 + 63, { align: 'center', font: F12, color: SEC, a: la });
          }
        ...
        // (lifted a little so its last line always clears the chart title by ≥ 24 px, also at 1280×720)
        const gxp = !m ? X(L.gl[0]) : X(0), gyp = !m ? (short || md ? Math.max(126, Math.min(Y(L.gl[1]) - 6, y0 - 146)) : Math.min(Y(L.gl[1]) - 6, y0 - 146)) : Y(L.gl[1]);
      ...
      // left edge of the header block (the scene's thesis line wraps rather than run into it)
      ...
      function drawChrome() {
        ...
        if (!G.phone) {
          ...
          navHit.push({ x: 0, y: y - 22, w: cv.w * 0.3, h: 40, act: 'prev' });
          navHit.push({ x: cv.w * 0.7, y: y - 22, w: cv.w * 0.3, h: 40, act: 'next' });
          navHit.push({ x: cv.w * 0.3, y: y - 22, w: cv.w * 0.4, h: 40, act: 'play' });
        }
      }
      ...
      /* ---------------------------------------------------------------- frame */
      // tall (iPad portrait): the shorter steps (Plan, Capacity, Horizon) drop by a few design units so each
      // step's picture is centred in the band, gliding with the step's crossfade
      const TALL_DY = [0, 0, 0, 0, 72, 38, 44];
      function draw() {
        cv.clear();
        ctx.lineCap = 'butt'; ctx.lineJoin = 'miter';
        if (G.lay === 'tall') {
          const tr = reduced ? 1 : eio((rt - S.tSwitch) / 0.75);
          G.oy = G.oy0 + lerp(S.from >= 0 ? TALL_DY[S.from] : TALL_DY[S.step], TALL_DY[S.step], tr) * G.s;
        }
        const views = [view0, view1, view2, view3, view4, view5, view6];
        drawEnvLayer();
        for (let i = 0; i < 7; i++) { const a = sAlpha(i); if (a > 0.004) views[i](a, i === S.step ? age() : 9); }
        drawGhost();
        drawChrome();
        drawCaret();
        ctx.globalAlpha = 1;
      ...
      function choreo(dt) {
        ...
        if (S.step === 5) {
          if (!S.rungUser) {
            // the guide walks the ladder (deep → linear), then settles on MLP∘LTI(k): the +57 pt gap
            const b = t / BAR;
            let target = 0;
            if (b >= 1.4 && b < 4) target = clamp(Math.floor((b - 1.4) / 0.42), 0, 5);
            else if (b >= 4) target = 3;
      ...
      function setStep(i, user) {
        ...
        if (i === 5) { S.rungView = S.rung; if (!S.rungUser) { S.rung = 0; S.rungView = 0; sliders.rung.set(0); } }
      ...
      function showSlider(step) {
        const which = step === 2 ? 0 : step === 3 || step === 4 ? 1 : step === 5 ? 2 : -1;
        sEls.forEach((s, i) => {
          const on = i === which;
        ...
        slot.style.visibility = which < 0 && G && G.phone ? 'hidden' : '';
      }
      api.links([
      ...
      function myHint(text, ms) { api.hint(text, ms); myHintUntil = performance.now() + ms; }
      ...
      function togglePlay() {
        ...
        simRunning();
        myHint(S.playing ? 'Playing' : fitHint(['Paused · space to resume', 'Paused · [space]', 'Paused']), 1200);
      }
      ...
      function reset() {
        ...
        stepper.set(0); api.caption(CAPS[0]); showSlider(0);
        ...
        hookShown = 0; horS.col = 0; horS.tb = null; horS.fin = false;
