   ...
   Everything is synthesized (no samples) and routed through one master chain:
     ...
     bus(name) ─┬─ [duck] ─────────────────────────┐
                ...
                └─ delaySend ──► delay ──┴─► master ─► HP 30 Hz ─► de-harsh bell
                                   ─► air shelf ─► glue comp ─► trim ─► limiter
...
(function () {
  ...
  var DATA_PITCH = [86, 88, 90, 93, 95, 98]; // D6 E6 F#6 A6 B6 D7
  var srcCache = {};
  ...
  function renderSources(sr) {
    ...
    for (i = 0; i < n; i++) {
      ...
      var h2 = 0.07 * (0.6 + 0.4 * Math.sin(TAU * 0.37 * t)) * Math.sin(2 * p1 + 0.9);
      pure[i] = (0.6 * Math.sin(p1) + 0.4 * Math.sin(p2) + h2 + 0.025 * Math.sin(3 * p1 + 0.3)) *
        (0.88 + 0.12 * Math.sin(TAU * 0.53 * t + 1.1));
      var I1 = 0.18 + 0.72 * (0.5 + 0.5 * Math.sin(TAU * 0.23 * t)) * (0.55 + 0.45 * Math.sin(TAU * 0.61 * t + 1.3));
      var I2 = 0.08 + 0.34 * (0.5 + 0.5 * Math.sin(TAU * 0.17 * t + 2.1));
    ...
    for (i = a; i < b; i++) {
      t = (i - a) * dt;
      var idx = 1.4 + 1.2 * Math.sin(TAU * 37 * t);
  ...
  function createEngine(ctx, opts) {
    ...
    var deharsh = F('peaking', 4300, 0.9); deharsh.gain.value = -1.5; // silky, not spiky, presence
    var air = F('highshelf', 10500); air.gain.value = 1.5;             // a little "air" for the micro-sounds
    ...
    chain(master, hpf, deharsh, air, glue, trim, limiter, clip, out, ctx.destination);
    var analyser = ctx.createAnalyser();
    analyser.fftSize = 2048;
    analyser.smoothingTimeConstant = 0.6;
    out.connect(analyser);
    E.master = master; E.out = out; E.analyser = analyser;
    E.glue = glue; E.limiter = limiter;
    ...
    function makeIR(seconds, preDelay) {
      ...
      for (var ch = 0; ch < 2; ch++) {
        ...
        for (var i = pre; i < len; i++) {
          ...
          // damping: brightness decays faster than level (11 kHz → 1.8 kHz)
          var fc = 1800 + 9200 * Math.exp(-t / 0.4);
    ...
    var reverb = G(1);
    var rvHP = F('highpass', 320, 0.6), rvLP = F('lowpass', 7800, 0.5);
    ...
    var fb1 = G(0.3), fb2 = G(0.3), dlLP = F('lowpass', 4600, 0.5);
    ...
    function Voice(o, def) {
      ...
      if (rv > 0) { var rs = G(rv); pan.connect(rs); rs.connect(reverb); v.nodes.push(rs); }
      if (dl > 0) { var ds = G(dl); pan.connect(ds); ds.connect(delay); v.nodes.push(ds); }
    ...
    function clickInto(v, to, t, freq, q, amp, pan) {
      ...
      q = clamp(q, 0.3, Math.max(0.5, Math.PI * freq * 0.018)); // ring ≤ ~18 ms
    ...
    function tickInto(v, to, t, midi, dur, amp, pan) {
      var f = midiToHz(midi), d = clamp(dur, 0.0015, 0.012);
    ...
    function bitInto(v, to, t, midi, dur, amp, pan, kind, toMidi, lpHz) {
      ...
      s.connect(g);
      var last = g;
      if (lpHz !== 0) { var lp = v.n(F('lowpass', num(lpHz, 8200), 0)); g.connect(lp); last = lp; }
      panTo(v, last, pan, to);
      v.src(s, t, t + dur + 0.004);
    }
    // Micro hat: high-passed white noise, 5–25 ms.
    function hatInto(v, to, t, dec, hpF, amp) {
      var s = noiseSrc(WHITE);
      var hp = v.n(F('highpass', hpF, 2)), pk = v.n(F('peaking', Math.min(hpF * 1.3, 15000), 1.1));
      pk.gain.value = 3;
      var g = v.n(G(0));
      var stop = perc(g.gain, t, 0.0003, dec / 3.5, amp);
      s.connect(hp); hp.connect(pk); pk.connect(g); g.connect(to);
    ...
    function Cloud(to, prm, pool, win) {
      prm = prm || {};
      var P = {}, ramps = {}, k;
      for (k in CLOUD_DEF) P[k] = clamp(num(prm[k], CLOUD_DEF[k]), CLOUD_LIM[k][0], CLOUD_LIM[k][1]);
      var cur = list(pool); if (!cur.length) cur = [74, 76, 78, 81, 83, 86];
      ...
      function pitchAt(t) {
        ...
        var m = pick(src);
        if (Math.random() < val('octave', t)) m += Math.random() < 0.65 ? 12 : -12;
        return m;
      }
      cl.pump = function (h) {
    ...
    // One digital click / "bit" of texture. { freq (Hz, default random 2–9 kHz), q, gain, pan }
    P.click = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.15, reverb: 0, delay: 0.04 });
      clickInto(v, v.out, v.t, num(o.freq, rand(2000, 9000)), num(o.q, rand(1.4, 3.5)), 1);
      v.commit();
    };

    // Tuned micro tick (2–8 ms resonant burst). Use high octaves: degree(i, 2..3).
    P.tick = function (midi, o) {
      o = o || {}; midi = num(midi, NaN); if (midi !== midi) return;
      var v = Voice(o, { level: 0.15, reverb: 0.03, delay: 0.08 });
      tickInto(v, v.out, v.t, midi + rand(-0.03, 0.03), num(o.dur, 0.005), 1);
      // a whisper of broadband edge so it reads as a "tick", not a tone
      clickInto(v, v.out, v.t, 9000, 0.9, 0.12);
      v.commit();
    };
    ...
    P.bit = function (midi, o) {
      o = o || {}; midi = num(midi, NaN); if (midi !== midi) return;
      var v = Voice(o, { level: 0.13, reverb: 0.02, delay: 0.06 });
      ...
      bitInto(v, to, v.t, midi, clamp(num(o.dur, 0.022), 0.005, 0.2), 1, null, kind, num(o.to, NaN));
      v.commit();
    };
    ...
    P.data = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.1, reverb: 0.03, delay: 0.07 });
      ...
      var lp = v.n(F('lowpass', 8200, 0)); lp.connect(v.out);
      ...
      while (tt < t + dur && n++ < 320) {
        ...
        if (r < pc) clickInto(v, v.out, tt, rand(2400, 9000), rand(1.4, 4.5), a, pan);
        else if (r < pc + pb) bitInto(v, lp, tt, pick(pool), rand(0.006, 0.02), a * 0.85, pan, pick(BIT_KINDS), NaN, 0);
        else tickInto(v, v.out, tt, pick(pool) + 12, rand(0.003, 0.006), a, pan);
        tt += Math.max(0.003, expRand() / dens);
      }
      v.commit();
    };

    // One windowed glassy grain (15–120 ms). { dur, bright 0..1, gain, pan }
    P.grain = function (midi, o) {
      ...
      var lp = v.n(F('lowpass', 2200 * Math.pow(6, br), 0));
      lp.connect(v.out);
      var win = dur > 0.1 ? WIN_TUKEY : WIN_HANN;
      spawnGrain(lp, t, midi, dur, 1 - br * 0.55, 0, false, win, v);
      if (br > 0.02) spawnGrain(lp, t, midi, dur, br * 0.8, 0, true, win, v);
      v.commit();
    };
    ...
    P.glitch = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.2, reverb: 0.03, delay: 0.08 });
      ...
      var pitched = isFinite(o.midi), rate = pitched ? Math.pow(2, (o.midi - GRAIN_REF) / 12) : 1;
      ...
      var lp = v.n(F('lowpass', 9000, 0)); lp.connect(to);
      var tt = t, l = len;
    ...
    P.ratchet = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.11, pan: rand(-0.25, 0.25), reverb: 0, delay: 0.05 });
      var t = v.t, n = clamp(Math.round(num(o.count, 4)), 2, 16), span = clamp(num(o.span, E.stepDur()), 0.02, 2);
      var up = o.dir !== 'down', f0 = rand(7200, 8400);
      for (var k = 0; k < n; k++) {
        var x = k / (n - 1), a = up ? 0.35 + 0.65 * x : 1 - 0.65 * x;
        hatInto(v, v.out, t + (k * span) / n, 0.008 + 0.004 * (up ? x : 1 - x), f0 * (1 + 0.25 * x), a);
      }
      v.commit();
    };

    // Clean sine sub pulse (+ a hint of 2nd harmonic for small speakers). { dur }
    ...
    P.kick = function (o) {
      ...
      if (c > 0) {
        clickInto(v, v.out, t, 3600, 0.8, 0.3 * c);
        clickInto(v, v.out, t + 0.0004, 9500, 1.2, 0.08 * c);
      }
      v.commit();
    };

    // Micro hat: 5–25 ms high-passed noise with randomized colour and pan. {open:true} ≈ 60–120 ms.
    P.hat = function (o) {
      o = o || {};
      var open = !!o.open;
      var v = Voice(o, { level: open ? 0.13 : 0.2, pan: rand(-0.3, 0.3), reverb: open ? 0.05 : 0, delay: 0 });
      var dec = clamp(num(o.decay, open ? rand(0.07, 0.11) : rand(0.012, 0.025)), 0.004, 0.5);
      hatInto(v, v.out, v.t, dec, rand(4800, 7600) * num(o.tone, 1), 1);
      clickInto(v, v.out, v.t, rand(4600, 6800), rand(2, 3.5), open ? 0.25 : 0.4); // digital edge
      v.commit();
    };
    ...
    P.snare = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.24, reverb: 0.06, delay: 0.03 });
      var t = v.t, dec = clamp(num(o.decay, 0.045), 0.015, 0.4);
      var cr = o.crush === true ? 1 : o.crush === false ? 0 : num(o.crush, 0.2);
      var mix = v.n(G(1)), to = mix;
      if (Math.random() < cr) {
        var sh = v.n(SHAPER(CURVE_CRUSH3)), lpc = v.n(F('lowpass', 9000, 0));
        mix.connect(sh); sh.connect(lpc); lpc.connect(v.out);
      } else mix.connect(v.out);
      var s = noiseSrc(WHITE), hp = v.n(F('highpass', 1900, 0)), bp = v.n(F('peaking', 3400, 1.1));
      bp.gain.value = 3;
      var ng = v.n(G(0));
      var stop = perc(ng.gain, t, 0.0005, dec / 3.2, 0.55);
      s.connect(hp); hp.connect(bp); bp.connect(ng); ng.connect(to);
      v.src(s, t, stop);
      ...
      clickInto(v, to, t, 5200, 1.3, 0.35);
      v.commit();
    };
    ...
    P.shaker = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.1, pan: -0.22, reverb: 0.03, delay: 0 });
      var t = v.t, dec = Math.max(0.01, num(o.decay, 0.035)), atk = Math.max(0.002, num(o.attack, 0.006));
      var s = noiseSrc(WHITE);
      var bp = v.n(F('bandpass', 6800 * rand(0.93, 1.07), 1.3)), hp = v.n(F('highpass', 4200, 0));
      var g = v.n(G(0));
    ...
    P.pluck = function (midi, o) {
      ...
      var t = v.t, f = midiToHz(midi), dec = Math.max(0.05, num(o.dur, 0.32));
      var br = clamp(num(o.bright, 0.6), 0, 1);
      var car = O('sine', f), tri = O('triangle', f, 3), mod = O('sine', f * 3), mg = v.n(G(0));
      mg.gain.setValueAtTime(f * 3 * (0.4 + 1.3 * br), t);
      mg.gain.setTargetAtTime(0, t, 0.005 + 0.008 * (1 - br));
      mod.connect(mg); mg.connect(car.frequency);
      var gc = v.n(G(0.72)), gt = v.n(G(0.26));
      var lp = v.n(F('lowpass', 1000, 0));
      lp.frequency.setValueAtTime(Math.min(f * (4 + 7 * br), 12000), t);
      lp.frequency.setTargetAtTime(Math.max(f * 2, 700), t + 0.002, 0.04 + dec * 0.1);
      var amp = v.n(G(0));
      var stop = perc(amp.gain, t, 0.0015, dec * 0.3, 1);
    ...
    P.bell = function (midi, o) {
      ...
      var t = v.t, f = midiToHz(midi), dec = clamp(num(o.dur, 1.1), 0.1, 6);
      var ratio = num(o.ratio, 3), idx = num(o.index, 0.6);
      ...
      mod.connect(mg); mg.connect(car.frequency);
      var amp = v.n(G(0));
      var stop = perc(amp.gain, t, 0.004, dec / 5, 1);
      var lp = v.n(F('lowpass', Math.min(12000, f * 6), 0));
      car.connect(amp); amp.connect(lp); lp.connect(v.out);
      tone(v, lp, 'sine', f * 2, t, 0.003, dec / 14, 0.1, 0, 0, 3);
    ...
    P.keys = function (midi, o) {
      o = o || {}; midi = num(midi, NaN); if (midi !== midi) return;
      var v = Voice(o, { level: 0.2, reverb: 0.12, delay: 0.1 });
      var t = v.t, f = midiToHz(midi), dur = Math.max(0.05, num(o.dur, 0.9));
      var vel = clamp(num(o.vel, 0.8), 0, 1);
      var car = O('sine', f), car2 = O('sine', f, 6), mod = O('sine', f), mg = v.n(G(0));
      mg.gain.setValueAtTime(f * (0.3 + 0.45 * vel), t);
      mg.gain.setTargetAtTime(f * 0.08, t, 0.12);
      mod.connect(mg); mg.connect(car.frequency); mg.connect(car2.frequency);
      var g1 = v.n(G(0.62)), g2 = v.n(G(0.38)), amp = v.n(G(0));
      amp.gain.setValueAtTime(0, t);
      amp.gain.linearRampToValueAtTime(1, t + 0.005);
      amp.gain.setTargetAtTime(0, t + 0.005, 0.6 * Math.pow(440 / f, 0.3));
      amp.gain.setTargetAtTime(0, t + dur, 0.08);
      var stop = t + dur + 0.6;
      var lp = v.n(F('lowpass', 1000, 0)), hp = v.n(F('highpass', 140, 0));
      lp.frequency.setValueAtTime(Math.min(f * 5, 4200), t);
      lp.frequency.setTargetAtTime(Math.min(f * 2.6, 2600), t, 0.2);
      ...
      if (o.click !== false) clickInto(v, v.out, t, rand(3000, 4600), 2, 0.05 * vel);
      v.commit();
    };
    ...
    P.pad = function (midis, o) {
      ...
      var cutoff = clamp(num(o.cutoff, 1600), 200, 12000);
      ...
      for (var i = 0; i < notes.length; i++) {
        var fq = midiToHz(notes[i]), side = (i % 2 ? 1 : -1) * (0.25 + (0.4 * i) / notes.length);
      ...
      if (gd > 0) {
        ...
        while (tt < endG && count++ < 240) {
          var m = pick(notes) + (Math.random() < 0.7 ? 12 : 24);
    ...
    P.lead = function (midi, o) {
      ...
      var t = v.t, f = midiToHz(midi), dur = Math.max(0.05, num(o.dur, 0.5));
      var glide = Math.max(0.005, num(o.glide, 0.08));
      var a = O('sine', f), b = O('triangle', f, 4), mod = O('sine', f), mg = v.n(G(f * 0.25));
      if (isFinite(o.from)) {
        var f0 = midiToHz(o.from);
      ...
      var cut = clamp(num(o.cutoff, 3000), 300, 12000);
    ...
    P.noise = function (o) {
      ...
      var f = v.n(F(ftype, num(o.freq, 2000), num(o.q, 0.7)));
    ...
    // the pentatonic scale (rhythmic micro-sounds evolving into the downbeat). { dur, midi, octaves }
    P.riser = function (o) {
      o = o || {};
      var v = Voice(o, { level: 0.22, reverb: 0.12, delay: 0.1, prio: 1 });
      var t = v.t, dur = clamp(num(o.dur, 2), 0.1, 16), end = t + dur;
      var base = num(o.midi, KEY.root + 12), oct = clamp(num(o.octaves, 1.5), 0.25, 4);
      var s = noiseSrc(WHITE), bp = v.n(F('bandpass', 400, 1.6)), ng = v.n(G(0));
      ...
      v.src(s, t, end + 0.03);
      if (HAS_PAN) { v.pan.pan.setValueAtTime(-0.4, t); v.pan.pan.linearRampToValueAtTime(0.4, end); }
      var notes = scaleNotes(base, base + 12 * oct);
      if (!notes.length) notes = [base];
      var tt = t, iv = Math.min(0.14, dur / 6), k = 0, times = [];
      ...
      for (var i = 0; i < times.length; i++) {
        ...
        tickInto(v, v.out, times[i], m + 12, 0.004, 0.25 + 0.6 * x, (i % 2 ? 1 : -1) * 0.5 * x);
      }
      v.commit();
    };
    ...
    P.impact = function (o) {
      ...
      v.src(sub, t, stop);
      var s = noiseSrc(WHITE), lp = v.n(F('lowpass', 6000, 0)), ng = v.n(G(0));
      ...
      [98, 93, 86, 81, 74].forEach(function (m, i) {
    ...
    P.blip = function (midi, o) {
      ...
      var t = v.t, f = midiToHz(midi), dur = Math.max(0.02, num(o.dur, 0.1));
      tone(v, v.out, 'sine', f, t, 0.002, dur / 3.5, 1, f * 1.03, 0.025);
      clickInto(v, v.out, t, Math.min(f * 4, 12000), 2, 0.08);
      v.commit();
    };
    ...
    /* ================================================================ UI SOUNDS */
    // Microsound UI: short, quiet, clinical. Peaks ≈ −34…−25 dBFS.
    var UI = {};
    ...
    UI.hover = function (o) {
      ...
      clickInto(v, v.out, t, rand(4200, 8200), rand(2.2, 5), 0.032 * rand(0.82, 1.1));
      if (Math.random() < 0.3) clickInto(v, v.out, t + rand(0.006, 0.013), rand(6500, 9500), 3, 0.01);
      v.commit();
    };
    // crisp detent: a 1–2 ms click pair
    UI.tick = function (o) {
      var v = uv(o, 1, 0), t = v.t;
      clickInto(v, v.out, t, 3300, 1.8, 0.04, -0.08);
      clickInto(v, v.out, t + 0.0095, 6200, 2.4, 0.022, 0.1);
      v.commit();
    };
    // two quick digital bits, upward
    UI.select = function (o) {
      var v = uv(o, 1, 0), t = v.t;
      bitInto(v, v.out, t, 86, 0.016, 0.045, -0.12, 's8');
      bitInto(v, v.out, t + 0.048, 93, 0.02, 0.042, 0.12, 's8');
      clickInto(v, v.out, t, 7000, 2, 0.016);
      v.commit();
    };
    // short upward spray of grains
    UI.open = function (o) {
      o = o || {};
      var v = uv(o, 1, 0), t = v.t, ms = [74, 78, 81, 86, 90];
      if (o.reverb == null) { var rs = v.n(G(0.14)); v.pan.connect(rs); rs.connect(reverb); }
      for (var i = 0; i < ms.length; i++) {
        spawnGrain(v.out, t + i * 0.027 + rand(0, 0.004), ms[i], rand(0.04, 0.055), 0.06 * (1 - i * 0.1), -0.45 + (0.9 * i) / (ms.length - 1), i % 2 === 1, WIN_HANN, v);
      }
      clickInto(v, v.out, t, 5200, 2, 0.012, -0.3);
      v.commit();
    };
    // short downward spray
    UI.close = function (o) {
      var v = uv(o, 1, 0), t = v.t, ms = [88, 83, 78, 74];
      for (var i = 0; i < ms.length; i++) {
    ...
    UI.type = function (o) {
      ...
      clickInto(v, v.out, t, rand(2600, 4300), rand(1.1, 1.8), 0.026 * a);
      var th = rand(160, 210);
      tone(v, v.out, 'sine', th, t + rand(0, 0.0015), 0.0008, rand(0.004, 0.007), 0.016 * a, th * 1.5, 0.006);
      if (Math.random() < 0.4) bitInto(v, v.out, t + rand(0.008, 0.016), pick([98, 100, 102, 105]), rand(0.006, 0.01), 0.008 * a, rand(-0.3, 0.3), pick(BIT_KINDS));
      v.commit();
    };
    ...
    var SCENE_VOICING = {
      1: { root: 38, up: [74, 78, 81, 86, 88] },   // D add9
      2: { root: 35, up: [71, 74, 78, 81, 83] },   // Bm7
      3: { root: 31, up: [71, 74, 78, 81, 86] },   // Gmaj9 (B D F# A over G)
      4: { root: 33, up: [69, 76, 81, 83, 88] },   // A sus2
      5: { root: 38, up: [78, 81, 86, 90, 93] }    // D, high and open
    };
    UI.transition = function (n, o) {
      ...
      wg.gain.setTargetAtTime(0.03, t, 0.05);
      ...
      for (var i = 0; i < 7; i++) {
        var m = sc.up[i % 5] + (i >= 5 ? 12 : 0);
        spawnGrain(v.out, t + 0.06 + i * 0.034 + rand(0, 0.006), m, rand(0.05, 0.08), 0.024 * (1 - i * 0.06), -0.5 + i / 6, i % 2 === 1, WIN_HANN, v);
      }
      // soft sub thump on the scene root
      tone(v, v.out, 'sine', midiToHz(sc.root + 12), t + 0.06, 0.003, 0.07, 0.035, midiToHz(sc.root + 12) * 1.4, 0.03);
      // landing tick
      tickInto(v, v.out, t + 0.06 + 7 * 0.034 + 0.02, sc.up[4] + 12, 0.005, 0.014, 0.35);
      v.commit();
    };
    E.ui = UI;
    ...
    function brightHz(b) { return 1800 * Math.pow(9, clamp(b, 0, 1)); }

    // Evolving grain cloud. { density (grains/s ≤ 90), pitch:[midis], dur (grain s), spread, bright,
    //   gain, jitter, detune (cents), octave (prob. of ±12), highpass, attack, dest, reverb }
    var GRAN_LEVEL = 0.3;
    E.granular = function (o) {
      ...
      var cl = Cloud(lp, o, o.pitch || o.midis, o.window === 'tukey' ? WIN_TUKEY : WIN_HANN);
      cl.nextT = c.t;
    ...
    E.drone = function (midis, o) {
      ...
      var ggain = c.n(G(0.55)), ghp = c.n(F('highpass', 300, 0));
      ggain.connect(ghp); ghp.connect(c.out);
      function grainPool(ns) { return ns.map(function (m) { return m + 12; }).concat(ns.map(function (m) { return m + 24; })); }
      var cl = Cloud(ggain, { density: num(o.grains, 5), dur: 0.2, spread: 0.85, bright: 0.25, jitter: 1, detune: 6, octave: 0.05 }, grainPool(notes), WIN_TUKEY);
      cl.nextT = c.t;
      var unpump = addPump(function (h) {
    ...
    E.wind = function (o) {
      ...
      function lpF() { return 900 + 3600 * I; }
      ...
      var ws = noiseSrc(PINK), wbp = c.n(F('bandpass', 900 + 700 * I, 9)), wg = c.n(G(0.3 * I));
      var wl = O('sine', 0.061), wld = c.n(G(220));
      ...
      return {
        set: function (p, ramp) {
          ...
          glideTo(wbp.frequency, 900 + 700 * I, r); glideTo(wg.gain, 0.3 * I, r);
        },
    ...
    var GV = {
      ...
      clouds: [[74, 76, 78, 81, 86], [71, 74, 78, 81, 83], [71, 74, 78, 81, 86], [69, 76, 81, 83, 88]],
      ticks: [[86, 88, 90, 93, 98], [83, 86, 90, 93, 95], [83, 86, 90, 93, 98], [88, 93, 95, 100, 105]],
      ...
      keys: {
        ...
        3: [[2, 71, 3], [8, 74, 7]],
        4: [[3, 78, 3], [6, 81, 3], [10, 83, 5]],
        5: [[2, 81, 3], [9, 78, 6]],
        6: [[3, 76, 3], [6, 81, 3], [10, 88, 5]],
        7: [[2, 83, 3], [8, 81, 4]]
      }
    };
    ...
    E.makeGroove = function (dest, o) {
      ...
      // sparkle: a quiet, continuous shimmer of tiny high grains (D6–A7) — the "bits" in the air
      var sin_ = G(1), shp = F('highpass', 1000, 0);
      sin_.connect(shp); shp.connect(micro);
      var SPARK_AMP = 0.24;
      var spark = Cloud(sin_, { density: 4, dur: 0.025, spread: 0.95, bright: 0.35, jitter: 1, detune: 3, octave: 0.3, amp: SPARK_AMP }, GV.ticks[0], WIN_HANN);
      ...
      function step(s, t) {
        ...
        spark.P.density = intro ? 3 + (s / 32) * 10 : full ? 13 : 10;
        ...
        if (pm > 0) {
          if (intro) {
            ...
            if (Math.random() < (bar === 0 ? 0.22 : 0.36)) P.click({ when: t + sd * rand(0, 0.5), dest: micro, gain: pm * rand(0.5, 0.95), pan: rand(-0.8, 0.8) });
            ...
            if (Math.random() < 0.18) P.click({ when: t + sd * rand(0, 0.6), dest: micro, gain: pm * rand(0.5, 0.9), pan: rand(-0.9, 0.9) });
          }
          // glitch ornaments
          if (bar === 3 && i === 13) P.glitch({ when: t, midi: 86, repeats: 5, len: 0.028, dest: micro, gain: pm * 0.55 });
          if (bar === 5 && i === 10) { P.bit(93, { when: t, dest: micro, gain: pm * 0.6, pan: -0.3, dur: 0.018 }); P.bit(98, { when: t + sd * 0.5, dest: micro, gain: pm * 0.55, pan: 0.3, dur: 0.02 }); }
        ...
        if (ph > 0) {
          if (i % 4 === 2) { var op = i === 14 && bar % 2 === 1 && !last; P.hat({ when: t, dest: drums, gain: ph * 1.1, open: op, decay: op ? undefined : rand(0.03, 0.045) }); }
          else if (i % 2 === 1 && Math.random() < (full ? 0.62 : 0.45)) P.hat({ when: t, dest: drums, gain: ph * rand(0.3, 0.55) });
          else if (full && i % 4 === 0 && Math.random() < 0.15) P.hat({ when: t, dest: drums, gain: ph * 0.3 });
          if (full && i % 4 !== 0) P.shaker({ when: t, dest: drums, gain: ph * [0, 0.7, 1.1, 1.4][i % 4] * rand(0.85, 1.1), pan: 0.3 });
